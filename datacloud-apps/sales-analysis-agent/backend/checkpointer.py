...
The OpenGauss-compatible implementation lives in the core engine:
    src/datacloud-agent/session/pg_opengauss.py

langgraph.json references this file as:
    "checkpointer": {"backend": "custom", "path": "./checkpointer.py:get_checkpointer"}

Note: the source path contains a hyphen (datacloud-agent) which Python cannot import
...
_pg_py_path = os.path.abspath(
    os.path.join(
        ...
        "../../../datacloud-agent/src/datacloud-agent/session/pg_opengauss.py",
    )
)
