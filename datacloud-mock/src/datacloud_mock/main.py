...
class _ExceptionLoggingASGIMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        try:
            ...
            tb = "".join(tb_lines)
            msg = f"\n[datacloud-mock] 错误栈 {method} {path}:\n{tb}"
            logger.error("Unhandled exception on %s %s:\n%s", method, path, tb)
            print(msg, file=sys.stderr, flush=True)
            # 同时写入文件，确保可追溯（debugpy/uvicorn 可能吞掉 stderr）
            try:
                from pathlib import Path
                _err_log = Path(__file__).parent.parent.parent / "error.log"  # datacloud-mock/error.log
...
async def _global_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    ...
    print(f"\n[datacloud-mock] 错误栈 {request.method} {request.url.path}:\n{tb}", file=sys.stderr, flush=True)
    try:
        from pathlib import Path
        _err_log = Path(__file__).parent.parent.parent / "error.log"
        with open(_err_log, "a", encoding="utf-8") as f:
            f.write(f"\n[datacloud-mock] {request.method} {request.url.path}:\n{tb}\n")
    except Exception:
        pass
    return JSONResponse(
        status_code=500,
