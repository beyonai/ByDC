    ...
    with open(path, "r", encoding="utf-8") as f:
        ...
        for row in r:
            ...
            if emp and emp not in ctx["valid_user_numbers"]:
                ...
                row["iwhale_cbm_name"] = ctx["user_number_to_name"].get(ctx["default_emp_no"], row.get("iwhale_cbm_name", ""))
    ...
    with open(path, "r", encoding="utf-8") as f:
        ...
        for row in r:
            ...
            if emp and emp not in ctx["valid_user_numbers"]:
                ...
                row["name"] = ctx["user_number_to_name"].get(ctx["default_emp_no"], row.get("name", ""))
...
def generate_bo_status_change(path_bo, path_out, ctx):
    ...
    for i, bo in enumerate(bos):
        ...
        changed_by = (bo.get("updated_by") or bo.get("iwhale_cbm_emp_no") or ctx["default_emp_no"]).strip()
        if not bo_id:
            continue
        # 每条商机至少 1 条：初始 -> 当前状态
    ...
    fieldnames = ["id", "bo_id", "opportunity_id", "status_before", "status_after", "change_remark",
                  "changed_by", "changed_time", "created_by", "created_time", "updated_by", "updated_time", "is_deleted"]
...
def main():
    ...
    doc.append("")
    doc.append("## 1. 修正说明")
    doc.append("- 基准：common 下 po_users（user_id、user_number/user_code）、po_organization（org_id）为有效集合。")
    doc.append("- 将 CRM 表中引用到的、不在上述集合中的用户工号替换为第一个有效 user_number；组织 ID 替换为第一个有效 org_id。")
    doc.append("")
    doc.append("## 2. 各表修正情况")
    ...
    doc.append("- 依据：修正后的 sales_business_opportunity 的 id、opportunity_id、business_opportunity_process、created_time/updated_time。")
    doc.append("- 每条商机生成 1 条状态变更记录：status_after = 当前商机状态，status_before 为空，change_remark 为「初始录入/状态同步」。")
