...
```mermaid
erDiagram
    SalesPerson ||--o{ SalesPersonKpiSummary : "个人KPI"
    SalesPerson ||--o{ SalesPersonKpiCompletion : "个人KPI完成统计"
    SalesPerson ||--o{ SalesPersonKpiDetail : "合同"
    SalesPerson ||--o{ SalesEmpAttendance : "考勤"
    SalesPerson ||--o{ SalesDailyReport : "日报"
    SalesPerson }o--|| OrganizationDepartment : "归属"
    
    OrganizationDepartment ||--o{ SalesPerson : "包含"
    OrganizationDepartment ||--o{ SalesOrgKpiSummary : "组织KPI"
    OrganizationDepartment ||--o{ SalesOrgKpiCompletion : "组织KPI完成统计"
    OrganizationDepartment ||--o{ OrganizationDepartment : "父级"
    OrganizationDepartment ||--o{ SalesPersonKpiDetail : "责任人组织"
    OrganizationDepartment ||--o{ SalesDailyReport : "归属组织"
    
    SalesPersonKpiDetail }o--|| OrganizationDepartment : "责任人组织"
   
    
    SalesBusinessOpportunity ||--o{ SalesBoStatusChange : "状态变更记录"
    SalesBusinessOpportunity }o--|| SalesPerson : "负责人"
    SalesBusinessOpportunity }o--|| OrganizationDepartment : "负责人组织"
    SalesBusinessOpportunity }o--|| SalesCustomer : "客户"
    
    SalesCustomer }o--|| SalesPerson : "维护人"
    SalesCustomer }o--|| OrganizationDepartment : "维护人组织"
    
    SalesPerson ||--o{ SalesExpenseReport : "申请人"
    OrganizationDepartment ||--o{ SalesExpenseReport : "申请组织"
    SalesExpenseReport }o--o| SalesBusinessOpportunity : "关联商机"
    SalesExpenseReport }o--o| SalesCustomer : "关联客户"
    
    SalesPerson ||--o{ TodoItems : "创建/发起"
    OrganizationDepartment ||--o{ TodoItems : "归属组织"
    
    SalesPerson ||--o{ TodoItemHandlers : "处理人"
    OrganizationDepartment ||--o{ TodoItemHandlers : "处理人组织"
    TodoItems ||--o{ TodoItemHandlers : "包含处理人"
    
    SalesPerson ||--o{ SalesMeetingNote : "创建/参与"
    ...
    SalesPersonKpiSummary {
    }

    SalesPersonKpiCompletion {
    }
    
    SalesPersonKpiDetail {
    }
...
1. **人员表(SalesPerson) ↔ 组织表**：人员通过org_id归属到组织，组织包含多个人员
2. **组织表(OrganizationDepartment) ↔ 组织表**：组织之间存在父子层级关系（通过parent_org_id）
3. **人员表(SalesPerson)  → 个人KPI表(SalesPersonKpiSummary)**：一个人员有多个个人KPI记录（通过emp_no或user_id关联）
4. **人员表(SalesPerson)  → 个人KPI完成统计表(SalesPersonKpiCompletion)**：根据个人合同按周/月汇总的KPI完成统计，一个人员有多条账期记录（通过emp_no关联）
5. **人员表(SalesPerson)  → 合同表(SalesPersonKpiDetail)**：一个人员有多个合同记录（通过emp_no或user_id关联）
6. **组织表(OrganizationDepartment)  → 合同表**：合同通过责任人组织ID关联到组织（通过emp_org_id关联到org_id）
7. **组织表(OrganizationDepartment) → 组织KPI完成统计表(SalesOrgKpiCompletion)**：根据组织及下属合同按周/月汇总的KPI完成统计，一个组织有多条账期记录（通过org_id关联）
8. **人员表(SalesPerson)  → 员工考勤表(SalesEmpAttendance)**：一个人员有多个考勤记录（通过emp_no或user_id关联）
9. **人员表(SalesPerson)  → 日报记录表**(SalesDailyReport)：一个人员有多个日报记录（通过belong_emp_no关联）
10. **组织表(OrganizationDepartment) → 日报记录表**：日报通过归属组织ID关联到组织（通过belong_emp_org_id关联到org_id）
11. **组织表(OrganizationDepartment) → 组织KPI表(SalesOrgKpiSummary)**：一个组织有多个组织KPI记录（通过org_id关联）
12. **商机表(SalesBusinessOpportunity) → 商机状态变更表(SalesBoStatusChange)**：一条商机有多条状态变更记录（通过 bo_id 关联 sales_business_opportunity.id）
13. **商机表(SalesBusinessOpportunity) → 人员表**：商机通过负责人工号关联到人员（通过iwhale_cbm_emp_no关联）
14. **商机表(SalesBusinessOpportunity)  → 组织表**：商机通过负责人组织ID关联到组织（通过iwhale_cbm_org_id关联）
15. **商机表(SalesBusinessOpportunity)  → 客户表**：商机通过客户名称关联到客户（通过customer_name关联）
16. **客户表(SalesCustomer) → 人员表**：客户通过维护人工号关联到人员（通过iwhale_cbm_emp_no关联）
17. **客户表(SalesCustomer)  → 组织表**：客户通过维护人组织ID关联到组织（通过iwhale_cbm_org_id关联）
18. **费用报备表(SalesExpenseReport) → 人员表**：费用通过申请人工号关联到人员（通过applicant_emp_no关联）
19. **费用报备表(SalesExpenseReport) → 组织表**：费用通过申请组织ID关联到组织（通过applicant_org_id关联）
20. **费用报备表(SalesExpenseReport) ↔ 商机表**：费用通过关联商机ID关联到商机（通过related_bo_id关联）
21. **费用报备表(SalesExpenseReport) ↔ 客户表**：费用通过关联客户名称/ID关联到客户（通过related_customer_id关联）
22. **待办事项表(TodoItems) → 人员表**：待办表通过创建人和发起人关联到人员（通过created_by/promoter关联）
23. **待办事项表(TodoItems) → 组织表**：待办表通过组织ID关联到归属组织（通过org_id关联）
24. **待办处理人表(TodoItemHandlers) → 待办事项表**：处理人表通过待办ID关联到主待办事项下（通过todo_item_id关联）
25. **待办处理人表(TodoItemHandlers) → 人员表**：处理人表通过处理人ID关联到人员（通过handler_id关联）
26. **待办处理人表(TodoItemHandlers) → 组织表**：处理人表通过组织ID关联到处理人所在的组织（通过org_id关联）
27. **会议纪要记录表(SalesMeetingNote) → 人员表**：会议纪要通过创建人获取相关的人员实体，同时可通过参与人员工号列表多态关联到对应的人员实体
28. **会议纪要记录表(SalesMeetingNote) ↔ 商机表**：会议纪要通过关联商机ID关联到对应商机实体
29. **会议纪要记录表(SalesMeetingNote) ↔ 客户表**：会议纪要通过关联客户ID关联到所属客户实体
...
##### 2.2.1 人员表(SalesPerson) 

```sql
CREATE TABLE `sales_person` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `user_id` varchar(32) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci DEFAULT NULL COMMENT '联系人用户id',
  `emp_no` varchar(32) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL COMMENT '员工工号',
  `org_id` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL COMMENT '组织ID',
  ...
  `emp_level` varchar(64) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci DEFAULT NULL COMMENT '职级',
  `emp_time` date NOT NULL COMMENT '入职时间',
  `status` varchar(64) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL COMMENT '状态',
  `base_city` varchar(64) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL COMMENT '驻地',
  ...
  `role` varchar(64) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL DEFAULT '' COMMENT '角色：SUPERVISOR-主管；COMMON-普通员工；OPERATION_SUPERVISOR-运管；PROBATION-试用期员工',
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE KEY `uk_emp_no` (`emp_no`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=126 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci ROW_FORMAT=DYNAMIC COMMENT='人员表';
```



##### 2.2.2 组织表(organization_department)
...
CREATE TABLE `organization_department` (
  ...
  `org_name` varchar(64) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL COMMENT '组织名称',
  `parent_org_id` varchar(32) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci DEFAULT NULL COMMENT '父组织ID',
  `org_index` tinyint(1) NOT NULL DEFAULT '0' COMMENT '组织排序索引',
  `supervisor_emp_no` varchar(32) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL DEFAULT '' COMMENT '组织主管工号',
  `created_by` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL COMMENT '创建人',
  `created_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_by` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci DEFAULT NULL COMMENT '更新人',
  `updated_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `is_deleted` int NOT NULL DEFAULT '0' COMMENT '逻辑删除标识(0:正常,1:删除)',
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE KEY `uk_org_id` (`org_id`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=29 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci ROW_FORMAT=DYNAMIC COMMENT='组织表';
```
...
##### 2.2.2.3 合同表(sales_person_kpi_detail)

```sql
CREATE TABLE `sales_person_kpi_detail` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
...
##### 2.2.2.6 个人kpi表(sales_person_kpi_summary)
...
##### 2.2.2.8 个人KPI完成统计表(sales_person_kpi_completion)

根据个人合同（sales_person_kpi_detail）统计个人 KPI 完成情况，按**周、月**两个账期落表，用于达成率、排名等分析。

```sql
CREATE TABLE `sales_person_kpi_completion` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `emp_no` varchar(32) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL COMMENT '责任人工号',
...
| 步骤2 | 销售主管 | 按以下操作，<br />1、输入："帮我导出整个「云智能公司」过去1年跟进的商机。"<br />2、系统：按当前用户权限过滤后，导出该组织过去1年跟进的商机列表，以表格或文件形式较快返回。 | 商机（sales_business_opportunity）、组织（organization_department） |
| 步骤3 | 任意角色 | 按以下操作，<br />1、输入："王小明作为销售是否优秀？"<br />2、系统：根据企业知识解析「优秀」标准，查询王小明的 KPI 完成排名及在公司/组织内占比，输出是否优秀的判定结论及依据摘要。 | 人员（sales_person）、个人KPI汇总（sales_person_kpi_summary）、商机（sales_business_opportunity）、合同表（sales_person_kpi_detail）、组织KPI（sales_org_kpi_summary）、企业知识/规章 |
| 步骤4 | 任意角色 | 按以下操作，<br />1、输入："帮我找出有合同成交的商机，并且成交额大于100万，成交的客户是政府行业的。"<br />2、系统：同库跨表联合检索后，以列表形式返回符合条件的商机，支持下钻到合同与客户明细。 | 商机（sales_business_opportunity）、合同表（sales_person_kpi_detail）、客户（sales_customer） |
| 步骤5 | 任意角色 | 按以下操作，<br />1、输入："帮我找出有合同成交的商机，并且成交额大于100万，成交的客户是政府行业的。并且看下这些商机负责人本周的考勤是否有异常。"<br />2、系统：跨库联邦计算后，返回上述商机列表，并附带各商机负责人本周考勤异常标记。 | 商机（sales_business_opportunity）、合同表（sales_person_kpi_detail）、客户（sales_customer）、员工考勤（sales_emp_attendance） |
...
| 步骤 | 角色 | 行为说明 | 关联数据实体 |
| ---- | ---- | ---- | ---- |
| 步骤1 | 销售员工 | 销售员工，王小明进行上班打卡。 | 人员（SalesPerson）、员工考勤（sales_emp_attendance） |
...
| 步骤 | 角色 | 行为说明 | 关联数据实体 |
| ---- | ---- | ---- | ---- |
| 步骤1 | 运营专员 | 业绩结果查询：<br />1、输入：“查询销售王小明本月的个人KPI指标和实际签订的合同金额，计算业绩完成率基准。”<br />2、系统：进行意图识别后，通过多表联合查询，返回其业绩达成情况。 | 人员表（sales_person）、个人KPI表（sales_person_kpi_summary）、合同表（sales_person_kpi_detail） |
...
| 步骤6 | 销售员工 | 自动化闭环追踪：<br />1、销售员工：业务人员在对赌期限内成功促成签约，并在系统录入成单状态。<br />2、系统：系统监测对应商机转化成功后，触发回调动作：自动冲销待办，并主动汇报“该商机对赌目标达成，建议返还此前可能扣留的绩效”。 | 商机表（sales_business_opportunity）、合同表（sales_person_kpi_detail）、待办事项表（todo_items）、待办处理人表（todo_item_handlers） |
...
| 步骤2 | 系统（Agent） | 过去效果总结：<br />1、系统：对比这些商机在处罚前后的转化金额分布、员工打卡记录与日报。输出复盘报告：“目前看来对赌政策在中小规模（小于50万）商机转化率提升40%；但对于100万级大型商机，业务员往往表现出悲观怠工情绪，日报更新锐减反而导致丢单加速。” | 商机表（sales_business_opportunity）、合同表（sales_person_kpi_detail）、考勤与日报 |
...
| ---- | ---- | ---- | ---- | ---- |
| 步骤1 | 销售总监 | 发起危机减损推演：<br />1、输入：“华南区大客户经理张某突然提交离职，以他目前手里的盘子，如果我们不做干预，推演一下本季度的损失。” | 人员表（sales_person） |  |
...
| 步骤3 | 系统（Agent） | 匹配推荐与方案生成：<br />1、系统：交叉检索同战区员工属性，给出接盘预案。<br />2、系统：“建议让【李大强】接管中国移动，让【王小明】接管剩余商机。” | 组织表（organization_department）、会议纪要表（sales_meeting_note）等 |  |
...
以 `sales_person` 表为例，给出与“王小明作为销售是否优秀”相关的样例数据：
...
以 `sales_person_kpi_detail`（合同表）为例，给出王小明负责的合同样例数据：
...
| 组织管理 | 组织名称            | 组织         | 组织             |      |      | 1、组织对象的名称字段（`organization_department.org_name`）  | 组织对象的人类可读名称                          | 部门名称、区域名称         |
...
| 对象建模 | 员工对象           | 对象     | 对象             |      | 1、员工对象           | 描述员工实体的本体对象，对应 `sales_person` 表               | 承载员工基本信息（工号、姓名、组织等）             |
| 对象建模 | 商机对象           | 对象     | 对象             |      | 1、商机对象           | 描述商机实体的本体对象，对应 `sales_business_opportunity` 表 | 承载销售机会数据                                   |
| 对象建模 | 合同对象           | 对象     | 对象             |      | 1、合同对象           | 描述合同/签单实体的本体对象，对应 `sales_person_kpi_detail` 表 | 承载已签订合同及金额等结果类数据                   |
| 对象建模 | 客户对象           | 对象     | 对象             |      | 1、客户对象           | 描述客户实体的本体对象，对应 `sales_customer` 表             | 承载客户属性与年度销售规模等信息                   |
| 对象建模 | 个人KPI对象        | 对象     | 对象             |      | 1、个人KPI对象        | 描述个人KPI实体的本体对象，对应 `sales_person_kpi_summary` 表 | 承载员工年度KPI目标与完成值                        |
