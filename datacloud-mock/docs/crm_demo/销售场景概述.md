...
CREATE TABLE `sales_bo_status_change` (
  ...
  `opportunity_id` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci DEFAULT NULL COMMENT '商机业务唯一标识，关联 sales_business_opportunity.id',
...
| 步骤  | 角色     | 行为说明                                                     | 关联数据实体 |
| ----- | -------- | ------------------------------------------------------------ | ------------ |
| 步骤7 | 任意角色 | 按以下操作，<br />1、上传一份 CSV 文件（如外部商机线索表或客户名单），文件可放在 mock 目录 `datacloud-mock/mock-resource/files/` 下供演示使用，参见示例文件 `external_opportunity_leads.csv`。<br />2、输入：「将刚上传的 CSV 与系统里已有商机、客户做融合分析，看哪些线索在系统里已有对应商机、哪些还没有跟进。」<br />3、系统：解析上传 CSV 的列（如客户名、预估金额、来源），与系统内商机表（sales_business_opportunity）、客户表（sales_customer）做匹配与融合，返回「已覆盖线索」与「未覆盖线索」的对比结果及简要统计。 | 上传 CSV（临时/外部数据）、商机（sales_business_opportunity）、客户（sales_customer） |

> 演示用 CSV 示例存放路径：`datacloud-mock/mock-resource/files/external_opportunity_leads.csv`。该文件包含若干外部线索（客户名、预估金额、来源、日期等），可与系统内商机、客户做融合分析。
...
| 步骤3 | 销售员工 | 按以下操作申请销售费用：<br />1、输入：帮我发起销售费用申请，理由是：明天宴请客户吃饭，关联商机是【dataCloud数智化建设】，关联【广东移动】客户，申请金额为1000元。<br />2、系统：<br />1）追问【广东移动】是指“中国*移动*通信集团*广东*有限公司”吗？<br />2）追问：【审批人】默认为 田刚【你的主管】<br />3、输入：是的。<br />4、系统：创建成功。<br / | 费用报备（sales_expense_report）、商机（sales_business_opportunity）、客户（sales_customer） |
...
| 步骤5 | 销售总监 | 调用技能与派发待办：<br />1、输入：“使用【生成人员画像】技能，帮我拉一下销售田刚的报告。”<br />2、系统：直接调用已固化的技能，一键快速返回田刚的人员画像长文报告。<br />3、销售总监审阅后，认为其本月客情异常，输入：“把田刚拜访量过低的问题，生成一个整改提醒的待办任务下发给他。”<br />4、系统：生成对应的整改待办任务自动派发。 | 以上实体及 待办事项表（todo_items）、待办处理人（todo_item_handlers） |
...
| ---- | ---- | ---- | ---- | ---- |
| 步骤1 | 销售总监 | 发起危机减损推演：<br />1、输入：“华南区大客户经理杜成鹏突然提交离职，以他目前手里的盘子，如果我们不做干预，推演一下本季度的损失。” | 人员表（po_users） |  |
| 步骤2 | 系统（Agent） | 流失风险推演：<br />1、系统：提取杜成鹏名下高价值商机及历史掉单率。<br />2、系统汇报：“若不立即响应，预计其名下的【中国移动】等3个核心客户的跟进将断档，本季度已预测回款的120万大概率滑单流失。” | 商机表（sales_business_opportunity）、客户表（sales_customer） | **掉单率计算口径**：基于商机表（sales_business_opportunity）。分子：该负责人（iwhale_cbm_emp_no）名下满足「未中标」的商机条数，即 win_bid=0；或商机状态（business_opportunity_process）为流标/丢单等终态的条数（具体枚举以业务配置为准）。分母：该负责人名下在统计周期内已结案或参与统计的商机总条数。掉单率 = 掉单商机数 / 总商机数（可按时段、客户等维度再细分）。 |
| 步骤3 | 系统（Agent） | 匹配推荐与方案生成：<br />1、系统：交叉检索同战区员工属性，给出接盘预案。<br />2、系统：“建议让【田刚】接管中国移动，让【王小明】接管剩余商机。” | 组织表（po_organization）、会议纪要表（sales_meeting_note）等 |  |
| 步骤4 | 销售总监 | 假设条件与模拟推演：<br />1、输入：“假如让【田刚】接管的话，他目前自己手上的大项目能兼顾得过来吗？换成【张恋】接管效果会不会更好？” | 人员表、KPI表、商机表 |  |
| 步骤5 | 系统（Agent） | 变量比较推演与派发：<br />1、系统：“推演显示：【田刚】目前虽然具备相似度，但他名下的项目正处冲刺期，同时接管可能会导致两个项目成功率均下降10%；<br />如果换作【张恋】：她的历史相似签单率为20%，且目前空余精力较多。综合评估换【张恋】接管能最大化保全120万的回款。<br />2、总监输入：“那就按张恋的方案下发紧急过渡交接待办！”<br />3、系统生成紧急任务落发执行。 | 待办事项表（todo_items） |  |
