...
- **唯一真源**：本体配置的**权威来源**在 **datacloud-data-service** 项目内维护；其它服务（含 datacloud）仅消费或通过适配层使用，不单独维护一份独立的本体/实体配置。
...
数据服务采用 **SDK + 服务** 分层架构（见第 10 章）：`datacloud_data_sdk` 与 `datacloud_data_service` 为同项目下 `src/` 内两个子包，无重复目录。逻辑层级如下：
...
| **SDK 核心层** | datacloud_data_sdk | `view.py`、`object.py`、`action.py`、`relation.py`、`context.py` | View/Object 实体；自然语言查询、自生说明、动作调用；InvocationContext 请求级上下文 |
| **SDK 本体层** | datacloud_data_sdk | `ontology/` | 本体解析（OWL/JSON/YAML → 内部模型）；OntologyLoader 产出 View/Object/Relation；术语集加载 |
| **SDK 计划层** | datacloud_data_sdk | `plan/` | ObjectViewPayload 构建；LLM 生成计划；计划校验；数据权限改写；步骤转执行任务 |
| **SDK 执行层** | datacloud_data_sdk | `executor/`、`sql_executor/` | 调度 API/SQL 步骤；连接池与 SQL 执行；结果转 CSV；从 InvocationContext 获取 token 等 |
| **SDK 聚合层** | datacloud_data_sdk | `aggregator/` | 按 aggregation 策略（DIRECT / SQLITE_MEM）产出最终 records |
| **SDK 支撑层** | datacloud_data_sdk | `events/`、`csv_storage/` | 事件总线、追踪；CSV 暂存管理 |

#### 6.9.2 代码目录树（完整）

SDK 与服务为**同项目内两个子包**，位于 `datacloud-data-service/src/` 下，无重复目录。
...
│           └── *.yaml
├── src/
│   ├── datacloud_data_sdk/                # SDK 子包（核心实体 + 执行 + 支撑，见第 10 章）
│   │   ├── __init__.py
...
│
├── tests/
│   ├── datacloud_data_sdk/               # SDK 单元/集成测试
...
**datacloud_data_sdk**（`src/datacloud_data_sdk/`）
...
采用**同项目内双子包**：`datacloud_data_sdk` 与 `datacloud_data_service` 为 `datacloud-data-service/src/` 下两个子包，无重复目录。执行层与支撑层（plan、executor、aggregator、events、csv_storage）均置于 SDK 内。

```
datacloud-data-service/
├── src/
│   ├── datacloud_data_sdk/                # SDK 子包（核心 + 执行 + 支撑）
