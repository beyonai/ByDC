...
数据服务在逻辑上分为以下层级，自上而下请求流入、自下而上依赖支撑：

| 层级 | 目录 | 职责简述 |
|------|------|----------|
| **接入层** | `api/` | 接收 HTTP/MCP 请求，鉴权、租户解析、路由分发；统一数据查询发布事件，操作类工具转发 ActionExecutor |
| **工具层** | `tools/` | 工具注册与列表；统一查询工具封装；操作类工具的 inputSchema 生成、参数映射、术语转换、动作执行编排 |
| **计划层** | `plan/` | 从本体构建对象视图；LLM 生成查询计划；计划校验与重试；数据权限改写；计划步骤转执行任务 |
| **执行层** | `executor/`、`sql_executor/` | 调度 API/SQL 步骤；连接池与 SQL 执行；结果转 CSV |
| **聚合层** | `aggregator/` | 按 aggregation 策略（DIRECT / SQLITE_MEM）产出最终 records |
| **本体层** | `ontology/` | 本体解析（JSON/YAML/OWL → 内部模型）；术语集加载与解析；Ontology* → datacloud 适配 |
| **支撑层** | `events/`、`csv_storage/` | 事件总线、追踪；CSV 暂存管理 |
...
│       │   ├── action_tool_generator.py   # 从 OntologyAction 生成工具定义与 inputSchema
│       │   ├── action_executor.py         # 操作类工具执行编排（参数流水线 → ApiExecTask）
│       │   ├── input_schema_builder.py    # params → MCP inputSchema 转换
│       │   ├── param_mapper.py            # arguments → 逻辑参数名 + mapping_path → 物理请求
│       │   └── term_resolver.py           # 术语值转换：标签/别名 → 标准 code（见 6.8.1、8.5.4）
│       │
│       ├── plan/                          # 计划层：查询计划流水线
│       │   ├── __init__.py
│       │   ├── models.py                  # QueryExecutionPlan、ObjectViewPayload、Step 等
│       │   ├── object_view_builder.py     # Ontology → ObjectViewPayload
│       │   ├── query_plan_generator.py    # LLM 生成计划
│       │   ├── plan_validator.py          # 计划校验
│       │   ├── data_permission_rewriter.py
│       │   └── execution_object_converter.py  # Plan steps → ApiExecTask/SqlExecTask
│       │
│       ├── executor/                      # 执行层：API/SQL 执行
│       │   ├── __init__.py
│       │   ├── executor.py                # 主执行入口，调度 API/SQL 步骤
│       │   ├── api_executor.py            # HTTP 调用（查询 steps + 操作类工具）
│       │   └── sql_executor_adapter.py   # 调用 sql_executor 或外部 SQL 服务
│       │
│       ├── sql_executor/                  # 执行层：数据库连接与 SQL
│       │   ├── __init__.py
...
│       │
│       ├── ontology/                      # 本体层：解析、模型、术语、适配
│       │   ├── __init__.py
│       │   ├── models.py                  # OntologyClass, OntologyField, OntologyRelation, OntologyAction, OntologyActionParam
│       │   ├── parser.py                  # OWL/JSON/YAML → 内部模型
│       │   ├── knowledge_service_client.py # 从知识服务拉取本体（可选）
│       │   ├── term_loader.py             # 术语集加载（code-label 及别名）
│       │   └── adapters/
│       │       ├── __init__.py
│       │       └── datacloud_adapter.py   # Ontology* → datacloud Entity/Field/Action；可选：校验桥接
│       │
│       ├── events/                        # 支撑层：事件驱动
│       │   ├── __init__.py
│       │   ├── bus.py
...
| | | query.py | POST /api/v1/query，发布 QueryRequestReceived |
| | | federated.py | POST /api/v1/query/federated |
| | | skills.py | GET /api/v1/skills/package |
| | | mcp_handler.py | MCP 入口；tools/call 按工具名分流（unified_data_query → 事件总线，action_code → ActionExecutor） |
| 工具层 | tools/ | registry.py | 工具列表（统一查询 + 操作类） |
| | | unified_query.py | 统一数据查询工具封装 |
| | | action_tool_generator.py | OntologyAction → 工具 name/description/inputSchema |
| | | action_executor.py | 操作类执行编排：参数流水线（映射→术语→校验）→ 构造 ApiExecTask → 调 executor |
...
| 计划层 | plan/ | models.py | QueryExecutionPlan、ObjectViewPayload、Step、Aggregation 等 |
| | | object_view_builder.py | 本体 → ObjectViewPayload（含 relations） |
| | | query_plan_generator.py | LLM 生成计划 |
| | | plan_validator.py | 步骤与引用校验 |
| | | data_permission_rewriter.py | 租户/权限注入 |
| | | execution_object_converter.py | Plan steps → ApiExecTask/SqlExecTask/AggregationTask |
| 执行层 | executor/ | executor.py | 调度 API/SQL 步骤，结果落 CSV |
| | | api_executor.py | HTTP 请求构造与调用 |
| | | sql_executor_adapter.py | 调用 sql_executor 或外部 SQL 服务 |
| 执行层 | sql_executor/ | models.py | SqlExecTask、DataSourceConfig |
...
| 聚合层 | aggregator/ | direct_aggregator.py | strategy=DIRECT，从指定 step 的 CSV 产出 |
| | | sqlite_aggregator.py | strategy=SQLITE_MEM，联邦 SQL 聚合 |
| 本体层 | ontology/ | models.py | OntologyClass、OntologyField、OntologyRelation、OntologyAction、OntologyActionParam |
| | | parser.py | OWL/JSON/YAML → 内部模型 |
| | | knowledge_service_client.py | 从知识服务拉取本体（可选） |
| | | term_loader.py | 术语集加载（code、label、aliases） |
| | | adapters/datacloud_adapter.py | Ontology* → datacloud Entity/Field/Action |
| 支撑层 | events/ | bus.py、events.py、handlers.py、tracing.py | 事件总线、事件类型、订阅注册、链路追踪 |
