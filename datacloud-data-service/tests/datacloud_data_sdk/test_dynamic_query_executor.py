...
def sqlite_loader() -> OntologyLoader:
    ...
    loader.load_from_content({
        "objects": [{
            ...
            "fields": [
                {"field_code": "id", "field_name": "主键ID", "field_type": "INTEGER", "source_column": "id"},
                {"field_code": "name", "field_name": "名称", "field_type": "STRING", "source_column": "name"},
...
@pytest.fixture
async def executor_with_data(sqlite_loader: OntologyLoader) -> tuple[DynamicQueryExecutor, OntologyLoader]:
...
@pytest.mark.asyncio
async def test_execute_db_no_aggregates(executor_with_data: tuple[DynamicQueryExecutor, OntologyLoader]) -> None:
...
@pytest.mark.asyncio
async def test_execute_db_with_filters(executor_with_data: tuple[DynamicQueryExecutor, OntologyLoader]) -> None:
    """带 filters 时使用参数化查询。"""
    executor, _ = executor_with_data
...
@pytest.mark.asyncio
async def test_execute_db_aggregates_meta_columns(executor_with_data: tuple[DynamicQueryExecutor, OntologyLoader]) -> None:
