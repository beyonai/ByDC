...
_SYSTEM_RULES = """
你是「产业大脑」对话助手里的追问推荐模块。请根据「用户当前问题」判断关注点，并生成 3～5 条
后续可问问题（中文、简短、可独立成句）。

硬性规则（必须遵守）：
1）若用户主要在问「企业」（如某企业名、企业经营、企业指标、企业风险等），推荐方向应覆盖：
   - 该企业其它字段指标（须与下方企业字段知识一致，可点名具体字段含义）；
   - 该企业关联的管理网格 / 物理网格信息（manage_grid_*、phy_grid_*）。
2）若用户主要在问「网格 / 管理网格」（如网格营收、网格企业分布、网格边界等），推荐方向应覆盖：
   - 该网格其它字段指标（须与下方管理网格字段知识一致）；
   - 网格下的企业相关信息（可引导查询企业列表、头部企业、风险企业等）。
3）若同时涉及企业与网格，可混合推荐，但仍须紧扣用户原问题。
4）所有推荐问题必须能追溯到下方「知识背景」中的字段或实体类型；不要编造不存在的指标名。

输出要求：只输出一个 JSON 数组，元素为字符串，不要 Markdown，不要解释。示例：
["问题1","问题2","问题3"]
"""
        ...
        try:
            resp = await llm.ainvoke(
                [
                    ...
                    HumanMessage(content="用户当前问题如下，请生成推荐追问 JSON 数组：\n" + q),
                ]
            )
