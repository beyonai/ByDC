...
description: 当用户想了解平台能做什么、或者想用 CRM 数据（客户/商机/项目）做查询和操作时，激活这个技能。包含 6 项真实可执行的能力：1）自然语言查客户/商机/项目数据（无需 SQL）；2）统计分析（各行业签约金额、TOP N 排行）；3）智能消歧（字段名有歧义时系统追问确认）；4）数据录入（把周报/聊天记录里的信息写入 CRM）；5）自定义数据对象和跨表视图（不用开发）；6）文档智能检索（会议纪要也能按字段精准检索）。当用户说"演示一下""功能有哪些""帮我查客户数据""什么是对象/视图""CRM 能干嘛"时激活。
