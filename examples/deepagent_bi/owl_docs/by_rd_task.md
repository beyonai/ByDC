...
**类型**：object
**描述**：研发任务对象，记录需求分析、产品设计、功能开发、测试、故障分析和故障处理任务。

## 查询能力（query）

研发任务对象，记录需求分析、产品设计、功能开发、测试、故障分析和故障处理任务。

按条件查询对象研发任务表的明细记录。**select / filters.field / order_by.field 统一使用对象属性编码**；支持字段过滤、排序、分页；不支持聚合统计。
...
| rd_task_type | 任务类型(1需求分析 2产品设计 3功能开发 4特性测试 5故障分析 6故障处理) | dimension | name | eq/in/is_null/is_not_null | self | - |  |
...
| rd_task_status | 任务状态(1处理中 2正常结束 3异常结束) | dimension | name | eq/in/is_null/is_not_null | self | - |  |
...
按规则对对象研发任务表做分组统计。**dimensions.field / metrics.field / filters.field 统一使用对象属性编码**；支持 dimensions + metrics + filters；不适合直接查看明细。
