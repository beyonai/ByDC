...
**类型**：object
**描述**：商机推进任务对象，记录商机跟进、商务应标、签约复盘等销售过程任务。

## 查询能力（query）

商机推进任务对象，记录商机跟进、商务应标、签约复盘等销售过程任务。

按条件查询对象商机任务表的明细记录。**select / filters.field / order_by.field 统一使用对象属性编码**；支持字段过滤、排序、分页；不支持聚合统计。
...
| opp_task_type | 任务类型(1线索获取 2方案交流 3商务报价 4商务应标 5商机签约 6应标复盘) | dimension | name | eq/in/is_null/is_not_null | self | - |  |
...
| opp_task_status | 任务状态(1处理中 2正常结束 3异常结束) | dimension | name | eq/in/is_null/is_not_null | self | - |  |
...
按规则对对象商机任务表做分组统计。**dimensions.field / metrics.field / filters.field 统一使用对象属性编码**；支持 dimensions + metrics + filters；不适合直接查看明细。
