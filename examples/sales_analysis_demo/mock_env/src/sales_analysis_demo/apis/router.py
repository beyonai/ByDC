...
from sales_analysis_demo.apis.po.router import router as po_router

router.include_router(po_router)

# Todo、Expense 依赖 redis/notice，按需挂载
try:
    from sales_analysis_demo.apis.todo.router import router as todo_router
    router.include_router(todo_router)
except ImportError as e:
    logger.warning("todo router skipped (missing deps): %s", e)

try:
    from sales_analysis_demo.apis.expense.router import router as expense_router
