...
async def setup() -> None:
    ...
    async with _init_lock:
        if _initialized:
            logger.debug("datacloud-agent already initialized – skipping.")
            return

        logger.info("datacloud-agent: starting SDK initialization …")

        # 1. Load & validate all env vars (raises ValidationError on misconfiguration).
        from datacloud_agent.config.env import Settings  # noqa: PLC0415

        settings = Settings()
        logger.info("datacloud-agent: environment variables validated.")
        ...
        await _pg_pool.open()
        logger.info("datacloud-agent: PG connection pool opened.")
        ...
        async with AsyncPostgresSaver.from_conn_string(settings.pg.checkpoint_uri) as saver:
            await saver.setup()
        logger.info("datacloud-agent: LangGraph checkpoint tables ready.")
        ...
        try:
            ...
            logger.info("datacloud-agent: datacloud-memory store initialized.")
        except ImportError:
            logger.warning(
        ...
        _initialized = True
        logger.info("datacloud-agent: initialization complete.")
...
def get_pg_pool() -> "AsyncConnectionPool":
    ...
    if not _initialized or _pg_pool is None:
        raise RuntimeError(
            "datacloud-agent is not initialized. "
...
async def teardown() -> None:
    ...
    if _pg_pool is not None:
        ...
        logger.info("datacloud-agent: PG connection pool closed.")
    _initialized = False
    _pg_pool = None
