# 测试规范 — datacloud-agent 专项

> **extends**: [`/docs/项目规范/TESTING_CONVENTIONS.md`](../../../docs/项目规范/TESTING_CONVENTIONS.md)（Monorepo 根规范，优先级更高）
>
> **适用范围**：`datacloud-agent` 子项目，测试目录 `tests/`
>
> **说明**：本文档**仅包含 datacloud-agent 特有内容**，通用测试规范（命名规则、类型划分、覆盖率方法论等）见根文档。
...
**路径推导**：把 `src/datacloud-agent/` 替换为 `tests/datacloud-agent/unit/`，文件名前加 `test_`。
...
| 文件 | 关键 Fixture | 说明 |
|------|-------------|------|
| `datacloud-agent/conftest.py` | `workspace_paths` | 临时目录 + DATACLOUD_WORKSPACE_* env 隔离 |
| `datacloud-agent/unit/conftest.py` | `StubSaver`, `mock_cursor`, `stub_saver` | OpenGaussSaver 单元测试桩 |
| `datacloud-agent/integration/conftest.py` | `initialized_sdk` | 调用 `bootstrap.setup()` / `teardown()` |
...
from datacloud_agent.session.pg_opengauss import OpenGaussSaver
...
- **不得修改 `tests/datacloud_agent/` 下的文件**（旧代码备份目录，只读）
