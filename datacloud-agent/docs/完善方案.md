...
**落地路径：**
- 新增 `datacloud_agent/llm/router.py`，暴露 `select_role(step_type, intent_context)` 接口。
...
- 定义 `datacloud_agent/orchestration/dag_schema.py` 强校验模型输出，并新增一套 `TemplateRegistry`（模板注册表/向量库）供 `intent` 匹配使用。
...
**新增 `datacloud_agent/context/builder.py` 统筹构建上下文**，严格保证列表拼接的顺序：
