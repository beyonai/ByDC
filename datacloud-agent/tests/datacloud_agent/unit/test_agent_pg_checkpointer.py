...
checkpointer lives in datacloud_agent.session.pg_opengauss and is injected by
...
def _load_agent_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location("datacloud_agent_hyphen_test", module_path)
...
def test_get_checkpointer_raises_when_uri_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    import asyncio
    from datacloud_agent.session.pg_opengauss import get_checkpointer
