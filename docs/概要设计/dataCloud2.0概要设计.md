...
Perplexity Computer、Claude Cowork、OpenClaw、Manus四大产品三周内密集更新，智能体操作系统争夺战全面升级。

- **Perplexity Computer / Claude Cowork / OpenClaw / Manus**：**个人 / 轻量端侧智能体**，主打「个人自动化、桌面操作、轻量办公」，**无企业级底座、无组织权限、无数据治理**。
- **阿里悟空 Agent**：**企业级智能体操作系统**，基于钉钉 CLI 化改造，**原生继承企业组织、权限、安全、数据底座**，是「AI 在企业里安全、可控、可审计地干活」的系统级入口。

| 维度           | Perplexity Computer / Claude Cowork / OpenClaw / Manus | 阿里悟空 Agent                                               |
| -------------- | ------------------------------------------------------ | ------------------------------------------------------------ |
| **核心定位**   | 个人 / 端侧智能体执行工具                              | 企业级智能体操作系统 + 生态入口                              |
| **企业级底座** | ❌ 无组织、无权限、无数据治理                           | ✅ 原生继承钉钉组织、权限、安全沙箱、全链路审计               |
| **执行方式**   | 模拟 GUI 点击、本地 / 云端执行                         | ✅ 钉钉 CLI 化改造，**原生操作企业核心能力**，非模拟点击      |
| **生态能力**   | 零散插件、无统一市场                                   | ✅ Token Hub 事业群，淘宝 / 1688 / 支付宝等 B 端能力以 Skill 接入，打造 ToB Skill 市场 |
| **行业落地**   | 通用场景、无行业方案                                   | ✅ OPT 十大行业解决方案，开箱即用，覆盖电商 / 跨境 / 财税等   |
| **安全合规**   | 弱、无企业级管控                                       | ✅ 企业级安全、权限继承、操作审计、Token 成本管控             |



### 0.2 总体定位

#### 定位：**行业智能体操作系统 = 数字孪生底座 + OpenClaw 级执行引擎**

| 架构层级                     | 核心组件                           | 核心能力                                                     | 解决的核心问题                                | 类比参考                                     |
| ---------------------------- | ---------------------------------- | ------------------------------------------------------------ | --------------------------------------------- | -------------------------------------------- |
| **上层：产品层（面向客户）** | 行业智能体（数字员工）             | 面向你的垂直行业（制造 / 能源 / 供应链等），封装行业技能、自动化流程。 | 企业 “最后一公里” 落地：AI 懂行业、能直接干活 | 钉钉 OPT 行业解决方案（但更深度）            |
| **中层：执行层（核心引擎）** | 企业级 OpenClaw（自研 / 二次开发） | 基于 OpenClaw 架构，做安全加固、多租户、权限管控、审计、私有化部署。<br />核心能力：网关调度、工具链、工作流、插件化、本地 / 云端执行。 | 解决 “AI 安全、可控、高效执行” 问题           | 企业版 OpenClaw\阿里悟空                     |
| **底层：底座层（护城河）**   | 数字孪生 + 本体建模（Ontology）    | 把企业设备、产线、流程、供应链映射为机器可理解的动态世界模型。<br />解决：**数据打通、业务可理解、全局决策、仿真推演（Palantir 核心）。** | 解决 “AI 看懂业务、全局决策” 问题             | Palantir Foundry 核心能力（轻量化 + 行业化） |
...
知识构建链路以**文件服务器**为统一本体与构建产物的存储中枢：由**本体构建引擎**完成构建并落盘；**本体推理引擎**与**百应管理门户**分别从文件服务器加载后进行各自侧的知识构建；业务人员在**百应管理门户**完成数字员工及对象、视图、动作等配置。
...
sequenceDiagram
    ...
    Note over 本体构建引擎,文件服务器: 阶段一：本体构建产出落盘
    本体构建引擎->>本体构建引擎: 执行本体构建、校验与打包
    本体构建引擎->>文件服务器: 上传构建文件夹（OWL/术语包/元数据等）
    文件服务器-->>本体构建引擎: 存储确认与访问标识

    Note over 文件服务器,百应管理门户: 阶段二：门户加载配置
    百应管理门户 ->> 文件服务器: 获取本体配置文件
    百应管理门户->>百应管理门户: 保存配置与元数据
	U ->> 百应管理门户: 配置数字员工(添加视图/对象/动作)
	百应管理门户 ->> U: 完成并发布
	
    Note over 文件服务器,本体推理引擎: 阶段三：推理引擎加载配置
    文件服务器 ->> 本体推理引擎: 下载/加载构建产物
    本体推理引擎-->>本体推理引擎: 知识构建(字典术语、列表术语、本体术语)
    本体推理引擎->>文件服务器: 完成构建
...
sequenceDiagram
    autonumber
    actor U as 用户
    participant 百应管理门户(JAVA)
	participant DataCloud数字员工
	participant 任务调度引擎
	participant 数据服务
	participant 知识服务
	participant 数据查询服务
	
	U ->> 百应管理门户(JAVA): 【@亦庄产业大脑】帮我查询全区范围有偷税漏税风险的企业
	百应管理门户(JAVA) ->> DataCloud数字员工: 转发
	DataCloud数字员工 ->> 任务调度引擎: 对用户问题进行拆解，调用视图/对象/动作[数据服务]
	
	任务调度引擎 ->> 数据服务: 拉取任务:Message_hanlder,指定视图/对象/动作
	数据服务 -->> 任务调度引擎: 返回思考过程：正在处理任务。
	数据服务 <<->> 知识服务: 根据用户问题查询相关知识
	数据服务 -->> 任务调度引擎: 返回思考过程：数据查询意图识别
	数据服务 <<->> 数据服务: 生成【查询计划】 及 【推荐问题】
	数据服务 -->> 任务调度引擎: 返回思考过程：生成数据查询计划
	数据服务 ->> 数据查询服务: 根据数据源编码，SQL进行查询。
	数据查询服务 ->> 数据服务: 返回查询结果
	数据服务 <<->> 数据服务: 根据数据结果卸载文件并组装返回结果
	数据服务 <<->> 任务调度引擎: 返回结果：数据 + 文件 + 推荐问题
	
	任务调度引擎 ->> DataCloud数字员工: 返回结果：数据 + 文件 + 推荐问题
	DataCloud数字员工 ->> 百应管理门户(JAVA): 返回结果
	百应管理门户(JAVA) ->> U: 返回结果
```
...
- [ ] V1：0328
  - [ ] 产品生态对接
    - [ ] 军华团队的本体构建协议对接协议(OWL)【黄升、郑方填】
    - [ ] 百应任务调度引擎对接，实现任务异步解构架构【黄升】
  - [ ] 产品能力开发(稳定性、准确率、性能)
    - [ ] 知识服务【罗彦卓】
      - [ ] 知识构建：根据OWL文件体系，构建术语和本体
      - [ ] 知识查询：根据用户问题，返回相关相关的知识。
    - [ ] 数据服务【邹海天】
      - [ ] 对接OWL协议：根据OWL执行数据查询。
      - [ ] 数据查询：根据自然语言、视图/对象，进行数据查询，并对大数据量卸载到文件。
      - [ ] 回答澄清：支持歧义数据查询的澄清。 
      - [ ] 派生属性：通过计算属性(绑定函数)，支持计算工时类的派生指标。
      - [ ] 大数据量：返回文件，支持分页获取数据(根据会话、文件路径)
      - [ ] 数据源执行：本阶段适配PG数据源。
    - [ ] 分析服务【黄升】
      - [ ] 问题改写：基于问题关联知识进行问题改写。
      - [ ] 引擎对接：对接百应任务调度引擎实现异步消息。
      - [ ] 空间对接：对接百应工作空间实现异构数据计算。
      - [ ] 流程整合：通过工作流整合知识服务和数据服务【本阶段提示词、技能固化】
  - [ ] 场景开发
    - [ ] 亦庄场景数据开发【重阳】
    - [ ] 【基于旧场景】亦庄本体开发，包括本体术语及本体OWL【吕隆鑫】
- [ ] V2：0404
  - [ ] 产品提升
    - [ ] 知识服务【罗彦卓】
      - [ ] 提供增量更新术语的API
    ...
    - [ ] 数据服务【邹海天】
      - [ ] 实现增量加载本体定义，提升加载性能。
    - [ ] 分析服务【黄升】
      - [ ] 问题推荐：基于上下文和术语关系，进行问题推荐。
      - [ ] 对接门户: 实现数字员工提示词、技能的动态挂载，实现门户界面更改配置，引擎侧自动生效。
      - [ ] 固化任务：允许用户把若干步骤固化成一个技能，到分析服务中。
  - [ ] 亦庄场景【公共】
    - [ ] 围绕亦庄场景的应用侧协议适配。
    - [ ] 围绕亦庄场景的准确率调优。
    - [ ] 围绕亦庄场景的性能调优。
