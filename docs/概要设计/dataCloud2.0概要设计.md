...
【OSDK、PSDK、branching 的详细阐述】



###### 1.2.1.3.2 MMDP(对标计算引擎)
...
| API分析师                                            | 1、内置各类可视化分析模板。<br />2、分支管理功能。           | [Types of analysis • Palantir](https://www.palantir.com/docs/foundry/analytics/types-of-analysis/) |
...
| **3. 决策推演**                   | **基于2B业务孪生的全局寻优与闭环：** 建立企业核心业务链,在虚拟世界中进行“What-if”沙盘推演，再进行决策。例如如果我增加10销售的话 ,业绩能否达成.如果实行对赌协议的话,业绩能够达成? | 没找到直接对得上的推演仿真平台.有一些零星新兴企业:simile\MiroFish | **离散事件仿真 (DES)** <br />**多智能体建模 (ABM / Multi-Agent)**<br />复杂系统仿真引擎 (Simulation) |
...
| 特色 | **生态化本体开发插件** | 1、基于cursor、Antigravity等IDE、git生态、open spec知识工程，提供本体智能开发能力。 | **评估指标**：<br />1、使用任意编码大模型+开发插件，可完成本体开发。 |
| 特色 | **智能调度引擎** | ~~1、分布式自驱：通过 Loopback 机制实现任务递归演进，无需外部轮询，解决同步调用在“数字员工”或“DataCloud”场景下易导致连接积压与超时。<br />2、控制与执行解耦：控制流（网关）与数据流（业务）彻底分离，能力插件化，解决网关逻辑与业务逻辑绑定，导致新能力接入速度慢、扩展性差。<br />3、全链路可观测：任务每一跳透明化追踪，实现故障秒级定位，解决多轮对话及异步任务上下文散落在业务侧，缺乏统一追踪。~~<br />1、计算资源调度：<br />2、异步数据计算：<br />3、计算过程监控：<br />计算引擎、分析引擎 | **评估指标**：<br />使用langchain、adk等框架构建Agent，直接实现异步调用Agent服务<br /><br />复杂<br />1、局部中断及响应。<br />2、安全<br />3、并行计算。<br />4、资源高效利用。<br />5、实时监控。 |
| 特色 | 渐进式知识增强 | 有多少参数，就。<br />渐进式的数据收集。<br />知识加载（术语加载）。让更智能体（低压力、），弱大模型、事务、上下文记忆丢失。 | antireSearch（渐进式加载） |
| 超越 | **基于术语的知识网络构建与应用** | **超越**：高效的数据意图方法<br />**说明**：<br />1、以术语为基础进行知识网络构建和应用，本体对象、视图、属性、方法、一切都是术语。<br />2、用MD文档描述逻辑，例如销售 有一个 “销售管理办法”的描述。<br />3、术语与术语之间可以额外添术语或普通文本标签。<br />1）以术语进行知识增强：知识冲突。（github是按行对比，我们以术语为中心进行对比）、知识关系<br />2）企业知识太多、知识质量。<br />3）基于关系可以找到动作。<br />4）cloude-Mem: 记忆机制（钩子），基于hook | **评估指标**：<br />1、用户的意图识别更准确。<br />2、用户问题转DSL更准确。<br />3、更轻量，例如业务规则基于MD来编写，不需要额外结构化治理。<br />4、消耗的时间、上下文更少。 |
| 超越 | **异构数据融合增强** | **超越**：基于本体术语进行结构化非结构融合<br />**说明：** <br />1、基于本体术语进行非结构化文档融检索。<br />2、基于本体实现非结构内容提取及融合。 | **评估指标**：<br />1、同时使用结构化、非结构语言进行数据检索。<br />2、快速把自然语言识别成一个结构化实体。 |
| 超越 | **资产自动沉淀和进化**           | **超越**：基于用户习惯洞察和沉淀数据知识<br />**核心差异**：<br />1、数据术语自动沉淀。<br />2、数据逻辑自动沉淀。<br />3、决策过程自动沉淀 | **评估指标**：<br />1、自我学习术语、数据查询逻辑、决策分析过程。<br />2、高频知识、数据的缓存机制。<br /> |
| 待定 | 待定 | 1、基于XX中间过程，再继续问。<br />2、点击一个数据，继续做探索。<br />3、notebookLLM的界面。<br />4、参考glean。 |                                                              |
...
![image-20260315232300686](assets/image-20260315232300686.png)
...
| 版本 | 模块                                                         | 描述 |
| ---- | ------------------------------------------------------------ | ---- |
| 0330 | 1.具备在线数据查询能力: 通过自然语言实现大小数据量, 异构对象的查询.<br />2.通过超级分析智能体整合数据服务,知识服务,跑通最小化流程. |      |
| 0430 | 1.实现数据分析模块并与周边生态整合: 与任务调度引擎对接, 本体构建引擎对接.<br />2.性能及准确率调优: 使用销售分析进行准确率和性能评测.<br />3.进行固化任务,记忆进化功能开发. |      |
| 0515 | 进行第一个版本开源.                                          |      |
| 0530 | 1.开发A2Ui,A2API分析模块.<br />2.提供智能开发插件的OSDK集成<br />3.进行准确率调优.<br />4.记忆自进化功能. |      |
| 0630 | 1. 推演仿真模块技术预研.                                     |      |
|      |                                                              |      |
