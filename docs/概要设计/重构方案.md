...
3. 扫描时忽略 `__init__.py`、`worker_plugins/`（Worker 侧插件包，非 hook 单文件）、`__pycache__`。
...
1. 插件异常默认直接失败。
2. 工具失败后，允许 after 插件做一次"可恢复拦截"（如降级结果）。
...
本章节已落地为目录化任务清单：每个批次一个子目录（`G01` 到 `G20`），每个目录内均包含 `README.md`（任务目标、范围、依赖、并行性、验收标准）。
...
1. `01-并行波次A`：`G01~G06`
2. `02-并行波次B`：`G07~G09`
3. `03-核心切换波次C`：`G10~G15`
4. `04-收口波次D`：`G16~G20`

索引文件：

- `docs/概要设计/重构剩余批次/README.md`

关键说明：

1. 旧节点下线（`intent.py` / `dag.py` / `clarification.py` / `loop.py` / `agent_delegate.py`）在 `G18/G19` 收口批次执行，不在前置批次直接删除。
2. 执行层 function-call ReAct 主改造在 `G10~G15`。
3. 结束节点执行摘要持久化在 `G16/G17`。
4. 文档与实现一致性修复在 `G06/G19`。
