...
> **字段值约定**：`select`、`filters.field`、`order_by.field` 均填写字段**中文名称**（本体属性的 `field_name`，如"企业名称"），执行层通过 `name_to_code` 反向映射自动转为物理列名，LLM 无需感知编码。

| 参数 | 类型 | 必填 | 默认值 | 说明 |
| ---- | ---- | ---- | ------ | ---- |
| `select` | `array[string]` | 否 | 空（返回全部字段） | 指定返回的字段中文名列表 |
...
**含义**：指定需要返回的字段，填字段**中文名称**（枚举值）。

| 项 | 值 |
| -- | -- |
| 类型 | `array[string]` |
| 元素取值 | 枚举，限定为对象声明的字段中文名，如 `["企业名称", "账期", "企业收入"]` |
| 为空时 | 返回对象全部字段 |

**LLM 填写规则**：从用户问题中提取明确需要的字段，直接填中文名。不确定时留空由系统返回全量字段。
...
**`name_to_code` 运行时转换**（执行层，LLM 不感知）：
```
"企业名称"       → enterprise_name → physical_col: ent_name
"账期"           → period          → physical_col: period_month
"企业收入"       → revenue         → physical_col: revenue_amt
"企业实际税负率" → tax_rate         → derived(expression): (total_tax / total_revenue * 100)
"亩产效益"       → yield_per_mu    → derived(expression): (total_revenue / grid_area / 10000)
```
...
| 字段 | 类型 | 必填 | 说明 |
| ---- | ---- | ---- | ---- |
| `field` | string | 是 | 字段**中文名称**（枚举，限声明字段） |
...
> ```sql
> -- LLM 填写：{"field": "企业实际税负率", "op": "gt", "value": 20}
> -- 执行层生成：
...
"filters": [
  ...
  {"field": "企业规模", "op": "in",      "value": ["大规模", "中规模"]},
  {"field": "企业名称", "op": "like",    "value": "科技"},
  {"field": "注销日期", "op": "is_null"}
]
```
...
> ```sql
> -- LLM 填写：{"field": "企业实际税负率", "direction": "desc"}
> -- 执行层生成：
...
```json
"order_by": [
  {"field": "企业收入", "direction": "desc"},
  {"field": "企业名称", "direction": "asc"}
]
```
...
```json
{
  "select":   ["企业名称", "账期", "企业收入", "企业规模"],
  "filters":  [
    {"field": "区域名称", "op": "eq",  "value": "亦庄"},
    {"field": "账期",     "op": "eq",  "value": "2026-01"},
    {"field": "企业收入", "op": "gte", "value": 5000000},
    {"field": "企业规模", "op": "eq",  "value": "大规模"}
  ],
...
| 规则 | 说明 | 错误示例 |
| ---- | ---- | -------- |
| 字段存在性 | 字段中文名必须在 `name_to_code` 映射中存在（对象声明过的字段） | `["不存在字段"]` → 报错 |
...
| 规则 | 说明 | 错误示例 |
| ---- | ---- | -------- |
| 字段存在性 | `field` 中文名必须在 `name_to_code` 映射中存在 | `{"field":"不存在","op":"eq","value":"x"}` → 报错 |
...
| 规则 | 说明 | 错误示例 |
| ---- | ---- | -------- |
| 字段存在性 | `field` 中文名必须在 `name_to_code` 映射中存在 | `{"field":"不存在字段","direction":"desc"}` → 报错 |
| 派生字段排序展开 | `property_kind=derived, mode=expression` 的字段在 ORDER BY 中重新展开表达式（不依赖 SELECT 别名，与 WHERE 同理） | `{"field":"企业实际税负率","direction":"desc"}` → `ORDER BY (total_tax/total_revenue*100) DESC` |
...
```
LLM 调用参数（中文字段名）
  → ① name_to_code 反查：field_name → field_code
...
LLM 填写字段中文名，SQL 生成前先做反向映射：

```python
# 构建映射：field_name（中文名）→ field_code（编码）
name_to_code = {f.field_name: f.field_code for f in cls.fields}

# LLM 传入 select = ["企业名称", "账期", "企业实际税负率"]
select_codes = [name_to_code.get(n, n) for n in select]
# → ["enterprise_name", "period", "tax_rate"]
```

filters / order_by 中的 `field` 同样先经过 name_to_code 转换。
...
def _build_where(filters, field_map, db_type, filter_relation="AND"):
    clauses = []
    for item in filters:
        fc   = item["field"]           # 已经过 name_to_code 转换
        f    = field_map[fc]
...
order_parts = []
for ob in order_by:
    fc        = name_to_code.get(ob["field"], ob["field"])
    f         = field_map.get(fc)
...
```mermaid
graph TB
    Input[query 调用入参] --> NameMap[name_to_code 反查\n中文名 → 字段编码]
    NameMap --> ParamCheck[参数校验\nvalidator.py]
...
{
  "filters": [
    {"field": "企业收入", "op": "gt", "value": 10000000},
    {"field": "企业税收", "op": "gt", "value": 2000000}
  ],
  "filter_relation": "OR",
  "limit": 20
}
...
```json
{
  "filters": [
    {"field": "账期",    "op": "eq", "value": "2026-01"},
    {"field": "企业收入", "op": "gt", "value": 5000000}
  ],
  "filter_relation": "OR"
}
```
...
**期望**：`VirtualActionValidationError`，`error_code=VIRTUAL_ACTION_ERR_UNSUPPORTED_FIELD`，消息："字段 '不存在字段' 不存在"。
...
> `所属园区名称` 的 `property_kind=linked`，需通过视图查询。

**期望**：`VirtualActionValidationError`，`error_code=VIRTUAL_ACTION_ERR_LINKED_NOT_SUPPORTED`，消息："字段 '所属园区名称' 为跨表关联字段，暂不支持，请使用对应的视图对象查询"。
...
| 字段 | 类型 | 必填 | 说明 |
| ---- | ---- | ---- | ---- |
| `field` | string | `agg≠count_all` 时必填 | 度量字段**中文名称** |
...
"metrics": [
  ...
  {"field": "企业税收",       "agg": "avg",  "as": "avg_tax"},
  {"field": "企业实际税负率", "agg": "avg",  "as": "avg_tax_rate"},
...
| 规则                    | 说明                                                         | 错误示例                             |
| ----------------------- | ------------------------------------------------------------ | ------------------------------------ |
| 字段存在性              | 中文名必须在 `name_to_code` 映射中存在                       | `["不存在字段"]` → 报错              |
...
| 规则 | 说明 | 错误示例 |
| ---- | ---- | -------- |
| 字段存在性 | `field` 中文名必须在 `name_to_code` 映射中存在 | `["不存在字段"]` → 报错 |
...
period_dim_fields = {
    ...
    if getattr(field_map.get(name_to_code.get(dim["field"], dim["field"])), "analytic_kind", None) == "period"
}
for mtr in metrics:
    if mtr.get("agg") == "sum":
        fc = name_to_code.get(mtr.get("field", ""), mtr.get("field", ""))
        f = field_map.get(fc)
...
输入参数均为**中文名**，执行层完成"中文→编码→物理列（或 formula）"的完整转换后生成 SQL。
...
###### ② 中文名→编码转换（name_to_code）

```python
name_to_code = {f.field_name: f.field_code for f in cls.fields}

# dimensions 中的 field 转换
for dim in dimensions:
    dim["field"] = name_to_code.get(dim["field"], dim["field"])

# metrics 中的 field 转换（count_all 无 field，跳过）
for mtr in metrics:
    if mtr.get("agg") != "count_all":
        mtr["field"] = name_to_code.get(mtr["field"], mtr["field"])

# filters 中的 field 转换（同 query_，此处略）
```
...
```json
{
  ...
  "filters":  [{"field": "账期", "op": "between", "value": ["2025-07", "2026-01"]}],
...
flowchart TD
    ...
    G -- 是 --> H[name_to_code 转换]
    H --> I[展开 SELECT / GROUP BY 含 formula 字段展开]
...
{
  "dimensions": [{"field": "区域名称", "group_op": "self"}],
  "metrics":    [{"field": "企业实际税负率", "agg": "avg", "as": "avg_tax_rate"}],
  "filters":    [{"field": "账期", "op": "eq", "value": "2026-01"}],
  "having":     [{"field": "avg_tax_rate", "op": "gt", "value": 10}],
  "order_by":   [{"field": "avg_tax_rate", "direction": "desc"}]
}
...
```json
{
  "dimensions": [{"field": "区域名称", "group_op": "self"}],
  "metrics":    [{"field": "企业收入", "agg": "sum", "as": "total_revenue"}],
  "having":     [{"field": "企业收入", "op": "gte", "value": 10000000}],
  "filters":    [{"field": "账期", "op": "eq", "value": "2026-01"}]
}
```
...
```json
{
  ...
  "filters":    [{"field": "账期", "op": "between", "value": ["2026-01", "2026-03"]}]
}
```
...
{
  "dimensions": [{"field": "账期", "group_op": "month"}],
  "metrics":    [{"field": "月初用户数", "agg": "max", "as": "max_users"}],
  "filters":    [{"field": "账期", "op": "between", "value": ["2026-01", "2026-03"]}]
}
```
...
{
  "dimensions": [{"field": "区域名称", "group_op": "self"}],
  "metrics":    [{"field": "月初用户数", "agg": "sum", "as": "region_users"}],
  "filters":    [{"field": "账期", "op": "eq", "value": "2026-01"}]
}
```
