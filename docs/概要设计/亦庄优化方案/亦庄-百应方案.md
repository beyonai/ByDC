...
#### 3.2.2 ontology_query方法

ontology_query是指本体的查询方法，对标sql的 select检索。

##### 3.2.2.0 名称及描述

1、名称：ontology_query_{对象/视图的编码}

2、描述：【待设计补充，需要 提供什么样的格式，最有利于让大模型在function call 精准调用】



##### 3.2.2.1 参数

【待补充，该方法到底有几个参数，参数名，参数描述，参数类型如何设置，最有利于让大模型在function call 精准调用】

1、select Fields

2、where Fields

3、whereType Fields =》全是and 还是全是or，二选一。
...
【待补充，根据参数和3.2.1的规则，如何对参数进行校验】。



##### 3.2.2.3 SQL生成规则 

【待补充，根据参数和3.2.1的规则，如何精准生成SQL，特别是虚拟标签类型，派生指标类型】



##### 3.2.2.4 SQL校验规则

【待补充，3.2.1的规则，如何校验生成的SQL，是否正确，特别是虚拟标签类型，派生指标类型】。



#### 3.2.3 ontology_compute方法

ontology_compute是指本体和统计计算方法，对标sql的 select 函数 group by d
