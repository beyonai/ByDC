...
以下方法新增到 `datacloud_knowledge/intent/` 子包，均为 async（psycopg3 异步连接），每个方法只做一件事。检索类方法均支持批量输入，内部用单条 SQL `WHERE ... IN (...)` 一次返回，避免 N 次往返：
...
async def search_by_exact_name(
    ...
    *,
    ...
    """批量精确匹配 term.term_name，返回 {word: [MatchCandidate]} 映射。
    单条 SQL WHERE term_name = ANY(:words)，置信度 1.0。
    """
```
...
async def search_by_alias_exact(
    ...
    *,
    ...
    """批量精确匹配 term_name.name_text，个人别名优先，返回 {word: [MatchCandidate]} 映射。
    单条 SQL WHERE name_text = ANY(:words)，置信度 0.95。
    """
```
...
async def search_by_bm25(
    ...
    *,
    ...
    """批量 BM25 检索，返回 {word: [MatchCandidate]} 映射。
    每个 word 独立计算 ts_rank，单条 SQL CROSS JOIN unnest(:words) 展开后聚合。
    """
```
...
async def search_by_vector(
    ...
    *,
    ...
    """批量向量检索，返回 {word: [MatchCandidate]} 映射。
    入参 key 为原始 word，value 为对应 embedding 向量。
    调用方负责在调用前将所有 word 批量转换为 query_vecs（一次 embedding API 调用）。
    内部对每个向量执行 name_vector <=> :vec 余弦检索，结果按 word 分组返回。
    """
```
...
async def create_term_name_async(
    ...
    *,
    ...
    """批量写入用户专属别名记录（term_name 表），返回 name_id 列表。
    records 每项为 (name_text, term_id, user_id)。
...
async def search_all_candidates(
    ...
    *,
    ...
    """四步漏斗批量检索所有 concept_terms，返回 {word: [MatchCandidate, ...]}。

    原子方法均支持批量，每步一条 SQL 覆盖所有词，无需 asyncio.gather 循环：
      step1: search_by_exact_name(words)       → 有结果的词直接确权，剩余词进 step2
      step2: search_by_alias_exact(words)      → 有结果的词确权/消歧，剩余词进 step3
      step3: search_by_bm25(words)             → 有结果的词进消歧，剩余词进 step4
      step4: embed(words) + search_by_vector   → 兜底，一次 embedding API 批量调用
    """
```
...
intent_node
  └─ search_all_candidates(concept_terms)     # tools/knowledge.py 组合方法
       ├─ search_by_exact_name(words)         # datacloud-knowledge 原子，批量一条 SQL
       ├─ search_by_alias_exact(words)        # datacloud-knowledge 原子，批量一条 SQL
       ├─ search_by_bm25(words)              # datacloud-knowledge 原子，批量一条 SQL
       └─ search_by_vector(query_vecs)        # datacloud-knowledge 原子，批量 embedding + 检索
