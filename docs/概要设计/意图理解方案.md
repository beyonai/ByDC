...
> 状态说明（历史归档）：本文档描述的是旧版多分支图（intent/clarification/direct_tool/dag/loop）设计。
> 当前主链路以 `knowledge_enhance -> planning -> execution -> end` 为准，请优先参考 `docs/概要设计/重构方案.md` 与最新 README。
...
```
name_keywords  tsvector,      — 用于bm25召回,单字索引
name_embedding vector(1024),  — 用于向量语义召回
search_scope   JSONB       NOT NULL DEFAULT '{}'::jsonb, -— 用于记录改名称被哪些用户，哪些组织拥有，数据样例 {"userid_200234": "200234"}
```
...
sequenceDiagram
    ...
    alt chitchat 或 clarify_needed
        ...
        INT->>DAG: 规划生成 plan
        DAG->>INS: 交由 execution 节点统一执行计划
    end

    INS->>User: 流式输出最终回答 SSE
    INS->>W: 向自身发 AskAgentCommand 术语反馈
    W->>DB: 异步更新 TermName.search_scope score
```
...
WHERE tn.name_text = :word
  AND (
    tn.search_scope->>'scope_user_id' IS NULL           -- 全局别名
    OR tn.search_scope->>'scope_user_id' = :user_id     -- 个人专属别名
  )
ORDER BY
  CASE WHEN tn.search_scope->>'scope_user_id' = :user_id THEN 0 ELSE 1 END,
  COALESCE((tn.search_scope->:user_id->>'score')::float, 0) DESC
LIMIT :top_k;
```
...
SELECT t.term_id,
       t.term_name,
       t.term_type_code,
       ts_rank(tn.name_keywords,
               plainto_tsquery('simple', :word))::float AS score,
       'bm25'        AS match_type,
       tn.name_text
FROM {schema}.term_name tn
JOIN {schema}.term t ON t.term_id = tn.term_id
WHERE tn.name_keywords @@ plainto_tsquery('simple', :word)
ORDER BY score DESC
LIMIT :top_k;
```
...
SELECT t.term_id,
       t.term_name,
       t.term_type_code,
       (1 - (tn.name_embedding <=> :query_vec::vector))::float AS score,
       'vector'      AS match_type,
       tn.name_text
FROM {schema}.term_name tn
JOIN {schema}.term t ON t.term_id = tn.term_id
WHERE tn.name_embedding IS NOT NULL
ORDER BY tn.name_embedding <=> :query_vec::vector
LIMIT :top_k;
```
...
##### datacloud-knowledge 已实现的原子方法

以下方法均已实现，`intent_node` 直接调用，无需新增：

**1. 术语检索：`match_mentions_with_search`**

`datacloud_knowledge/intent/matching.py#L101`
...
def match_mentions_with_search(
    ...
    search_mode: SearchMode = "strict",   # "strict" | "rapidfuzz" | "bm25" | "vector"
...
一个函数覆盖精确（strict）、BM25、向量三种模式，通过 `search_mode` 切换。漏斗逻辑由 `tools/knowledge.py` 的组合方法按步骤调用，每步传不同 `search_mode`。

**2. 术语关系检索：`_batch_query_subgraphs`**

`datacloud_knowledge/query/sql_engine.py#L1161`
...
批量多源 BFS，单条递归 CTE 覆盖所有 seed，用于消歧层2的拓扑辅助。

**3. 术语消歧：`disambiguate`**

`datacloud_knowledge/intent/disambiguation.py#L21`
...
执行置信度差值 + BFS 拓扑两层消歧，返回 `DisambiguationResult(confirmed, ambiguous)`。层3 LLM 消歧由 `tools/knowledge.py` 在此之后调用。

**4. 术语名称新增：`store_clarification_results`**

`examples/e_commerce_demo/mock_env/tests/type5_nl_knowledge_query/debug_intent.py#L142`（参考实现，需迁移至正式包）
...
遍历澄清结果，命中已有术语则调用 `create_user_term_name` 写别名；用户自定义描述则调用 `create_term_with_knowledge` 新建术语。返回 `name_id` 列表。

> 该函数目前在 examples 目录，需迁移至 `datacloud_knowledge/intent/storage.py` 作为正式 API。
...
async def search_all_candidates(
    ...
    *,
    user_id: str | None = None,
    session: Session,
    global_name_index: dict | None = None,
    user_cache: UserNameCache | None = None,
    embedding_service: EmbeddingService | None = None,
    top_k: int = 5,
) -> dict[str, list[MatchCandidate]]:
    """四步漏斗检索所有 concept_terms，返回 {word: [MatchCandidate, ...]}。

    每步调用 match_mentions_with_search，切换 search_mode：
      step1: search_mode="strict"    → 有结果的词直接确权，剩余词进 step2
      step2: search_mode="strict"    → 别名精确匹配（user_cache 中），剩余词进 step3
      step3: search_mode="bm25"      → 有结果的词进消歧，剩余词进 step4
      step4: search_mode="vector"    → 兜底语义检索（需 embedding_service）
    """
```

**3. 消歧（组合 `disambiguate` + LLM）**
...
async def disambiguate_candidates(
    ...
    *,
    ...
    层1+2：调用 disambiguate(match_result, session) — 置信度差值 + BFS 拓扑
    层3：  仍有歧义 → LLM 语境推理（见 4.1.4 层3提示词）
    """
```

**4. 别名新增（组合 `store_clarification_results`）**
...
async def save_clarification_results(
    ...
    *,
    ...
    内部调用 store_clarification_results（迁移自 examples 后的正式版本）。
    """
```
...
intent_node
  └─ search_all_candidates(concept_terms)          # tools/knowledge.py 组合方法
       └─ match_mentions_with_search(search_mode)  # datacloud-knowledge，按步骤切换模式
  └─ disambiguate_candidates(candidates_map)       # tools/knowledge.py 组合方法
       ├─ disambiguate(match_result, session)       # datacloud-knowledge，层1+2
       └─ llm.ainvoke(...)                          # LLM 消歧，层3

clarification_node
  └─ save_clarification_results(results, user_id)  # tools/knowledge.py 组合方法
       └─ store_clarification_results(...)          # datacloud-knowledge（迁移后）
```
...
持久化别名写入逻辑：
- 用户解释是已有术语的另一种叫法（如“企业=公司”）→ 新建 `TermName`，`search_scope` 打上 `scope_user_id`，初始 `score=1.0`
...
`TermName.search_scope` 中每个用户的 score 记录：
