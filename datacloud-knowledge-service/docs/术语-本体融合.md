...
用户希望将术语系统与现有的本体模型建立联系。具体需求：术语"王小明"应该作为"人员姓名"字段的实例，与"人员对象"的"人员姓名"字段建立联系。
...
**当前实现：**
- 本体字段和参数通过 `termMeta` 属性与术语关联
- `termTypeCode` 引用 TermType（术语类型）
- 术语元数据以 JSON 形式存储在本体节点属性中
- 但术语本身（Term）还不是本体中的节点
...
class EdgeLabel(Enum):
    ...
    # 字段-术语类型关系
    BOUND_TO = "BOUND_TO"                  # TermType -> Field
    BIND_TO = "BIND_TO"                    # Field -> TermType
```
...
**Phase 2: 术语-字段关联**
1. 支持 FieldNode 通过 BIND_TO 边关联到 TermType
