    ...
    subgraph AGENT_CORE["智能体核心层 (Agent Core/超级分析智能体)"]
        ...
        subgraph TOOLBOX["工具执行机制 (Tool Capabilities)"]
    		...
    		T_MEM_RECALL["🧠 recall_memory (向下渐进式穿透检索三层记忆体系)"]
             %% 数据基建系
            T_DATA_QUERY["🔍 data_query (穿透至数据底层获取业务明细碎片)"]
            
            %% 沙箱操纵的三剑客 (Sandbox 系)
            T_SBX_RUN["💻 sbx_run_code (在沙箱隔离区执行运算与推演)"]
            T_SBX_READ["📄 sbx_read_file (沙箱隔离工作区文件安全探测提取)"]
            ...
            T_KNOW_SEARCH ~~~ T_MEM_RECALL ~~~ T_DATA_QUERY ~~~ T_SBX_RUN ~~~ T_SBX_READ ~~~ T_SBX_WRITE ~~~  T_SKILL_BUILD ~~~ T_REPORT ~~~ T_EXTEND_TOLL
        end
