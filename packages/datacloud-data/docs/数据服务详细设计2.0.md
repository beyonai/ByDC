...
> **版本说明**：本文档为数据服务详细设计 2.0 版本，以**事件驱动架构**为核心设计原则，在 v1.0 基础上完善目录结构、强化事件流设计、整合接口规范，并修复已知问题。v2.1 补充 **SDK 与分层架构设计**（第 10 章）：datacloud-data-sdk、View/Object/Relation/Action、InvocationContext、执行层在 SDK 内、服务层使用 SDK 提供 MCP。
...
- [5 部署架构设计](#5-部署架构设计)
- [6 数据建模设计](#6-数据建模设计)
  - [6.1 本体文件来源与数据流](#61-本体文件来源与数据流)
  - [6.1.1 本体配置唯一真源与目录](#611-本体配置唯一真源与目录)
  - [6.1.2 data-service 内部本体模型（类 / 字段 / 关系 / 动作）](#612-data-service-内部本体模型类--字段--关系--动作)
  - [6.1.3 datacloud 融入与适配层](#613-datacloud-融入与适配层)
  - [6.1.4 OntologyLoader 使用与数据流](#614-ontologyloader-使用与数据流)
  - [6.9 架构层级与代码目录](#69-架构层级与代码目录)
    - [6.9.1 架构层级一览](#691-架构层级一览)
    - [6.9.2 代码目录树（完整）](#692-代码目录树完整)
    - [6.9.3 层级-目录-文件-职责表](#693-层级-目录-文件-职责表)
  - [6.10 动作执行详细设计](#610-动作执行详细设计)
    - [6.10.1 OntologyAction 与 OntologyActionParam 模型](#6101-ontologyaction-与-ontologyactionparam-模型)
    - [6.10.2 动作参数处理流水线](#6102-动作参数处理流水线)
    - [6.10.3 动作执行涉及的文件与层级](#6103-动作执行涉及的文件与层级)
- [7 数据库连接与 SQL 执行模块](#7-数据库连接与-sql-执行模块)
- [8 数据服务接口设计（MCP + REST）](#8-数据服务接口设计mcp--rest)
- [9 测试用例设计](#9-测试用例设计)
- [10 SDK 与分层架构设计](#10-sdk-与分层架构设计)
  - [10.1 整体架构与目录结构](#101-整体架构与目录结构)
  - [10.2 SDK 核心能力（datacloud-data-sdk）](#102-sdk-核心能力datacloud-data-sdk)
  - [10.3 对象关联（Relation）](#103-对象关联relation)
  - [10.4 数据流与 InvocationContext](#104-数据流与-invocationcontext)
  - [10.5 错误处理与测试补充](#105-错误处理与测试补充)
- [11 小结与实施建议](#11-小结与实施建议)
...
本文档在《dataCloud2.0_详细设计_服务拆分与接口》和《本体服务_模块设计》的基础上，**只针对数据服务（datacloud-data）** 给出可落地的详细设计，重点覆盖：
...
- **本体驱动**：从知识服务获取 OWL/JSON/YAML 本体元数据，作为配置驱动查询计划生成与执行；**本体配置的唯一真源在 data-service**，统一解析为内部本体模型（类/字段/关系/动作）。
- **datacloud 库级融入**：通过适配层将 data-service 的 Ontology\* 转为 datacloud 的 Entity/Field/Action，复用其校验与执行能力；datacloud 仅作为库引入（可裁剪 MCP 等无关模块），不单独维护实体配置。
...
```mermaid
flowchart LR
    Agent[超级分析智能体<br/>datacloud-analysis] --> DS[数据服务<br/>datacloud-data]
...
tracer = trace.get_tracer("datacloud-data", "1.0")
...
数据服务通过知识服务获取本体定义（OWL/JSON/YAML），解析后构建 **data-service 内部本体模型**（OntologyClass / OntologyField / OntologyRelation / OntologyAction），再据此生成 ObjectViewPayload；由大模型生成 QueryExecutionPlan，经校验、权限改写后执行；中间结果落 CSV，最终通过 SQLite 内存库联邦聚合产出 records。

#### 6.1.1 本体配置唯一真源与目录

- **唯一真源**：本体配置的**权威来源**在 **datacloud-data** 项目内维护；其它服务（含 datacloud）仅消费或通过适配层使用，不单独维护一份独立的本体/实体配置。
- **推荐目录**：在 data-service 中约定统一目录存放本体文件，例如：
  - `resources/ontology/` 或 `config/ontology/`：按「类/对象视图」划分文件（如 `employee.yaml`、`order.yaml`、`contract.yaml`）；
  - 每个文件描述：类名、字段、**对象间关联关系**、可用动作及动作执行所需的数据源/SQL 模板/HTTP 配置等。
- **格式**：支持 JSON / YAML / OWL；解析后统一转为 data-service 内部本体模型（见 6.1.2）。

#### 6.1.2 data-service 内部本体模型（类 / 字段 / 关系 / 动作）

在 `datacloud_data_service.ontology` 下定义**中立的本体模型**，作为对象视图、查询计划、执行任务生成的统一输入：

| 模型 | 说明 |
|------|------|
| **OntologyClass** | 类/对象：id、名称、所属数据源、表/API 绑定等。 |
| **OntologyField** | 字段/属性：name、type、aliases、约束、映射等。 |
| **OntologyRelation** | **对象间关联关系**（必选）：语义 + 可选技术细节。 |
| **OntologyAction** | 动作：id、参数、返回、与数据源/SQL/HTTP 的绑定及执行配置。 |

**OntologyRelation 设计（语义 + 技术细节，对应方案 C）**：

- **语义层**：关系类型如 `hasMany`、`belongsTo`、`oneToOne`、`oneToMany`、`manyToMany`；`source_class` / `target_class`；业务含义描述。
- **技术层（可选）**：在需要时附带：
  - `local_field` / `remote_field` 或 `join_keys`（fromField / toField）；
  - 同数据源下的 `join_sql` 片段或外键说明；
  - 跨数据源时的关联 key、CSV 表名等，供 ExecutionObjectConverter / Aggregator 使用。
- 对象视图（ObjectViewPayload）的 `relations` 由 OntologyRelation 转换而来；QueryPlanGenerator 与 PlanValidator 使用同一套关系模型生成与校验多表 join、多步执行。

#### 6.1.3 datacloud 融入与适配层

- **设计原则**：**本体的唯一权威表示是 data-service 的 Ontology\***（OntologyClass + OntologyField + OntologyRelation + OntologyAction）。**datacloud 侧不再维护一套平行的实体配置格式**；所有本体来源（含原 datacloud 的 `config/es_data`、`config/test/*.json` 等）**统一转化为** Ontology\* 对象后，再由 data-service 使用。
- **适配方向**：
  - **Ontology → datacloud**：通过 `datacloud_data_service.ontology.adapters.datacloud_adapter` 将 OntologyClass/Field/Relation 构造/装配为 datacloud 的 `Entity`、`Field` 及关系信息；将 OntologyAction 构造/装配为 datacloud 的 `Action` 或 action 配置；在需要校验、执行时调用 datacloud 的 core 能力（**仅引入 entity/field/action/storage 等，不引入 MCP 等无关模块**）。
  - **datacloud 既有配置 → Ontology**：通过迁移脚本或兼容 loader，将既有 JSON/YAML 配置**一次性或按需转换**为 Ontology\*，写入 data-service 的本体目录或通过知识服务暴露，保证“单一真源”在 data-service。
- **在事件链路中的落点**：
  - **ObjectViewBuilder**：从 Ontology 按 viewId/类名生成 ObjectViewPayload（含 relations），必要时通过适配层生成 datacloud Entity 做校验/组合。
  - **QueryPlanGenerator**：基于 ObjectView 与 OntologyAction 决定步骤与动作。
  - **ExecutionObjectConverter / Executor**：将计划步骤转为执行任务，并与 datacloud 的 storage / Action 执行逻辑绑定，实现「按类生成字段、属性、动作及动作执行」。

#### 6.1.4 OntologyLoader 使用与数据流

**OntologyLoader** 是本体层与核心层的桥梁：负责解析 OWL/JSON/YAML，产出 Ontology* 模型，并构建 View/Object/Relation/Action 供调用方使用。支持多种加载方式与两种使用层级。

##### 6.1.4.1 加载方式

| 方法 | 输入 | 适用场景 |
|------|------|----------|
| **load_from_path(path)** | 本地文件路径（.yaml / .json / .owl） | 开发、本地部署、resources/ontology/ 目录 |
| **load_from_service(view_id, object_ids)** | 知识服务 API，按 view_id 或 object_ids 拉取 | 生产、本体由知识服务集中管理 |
| **load_from_content(content, format)** | 内存中的 dict 或 RDF Graph，format 为 "json" / "yaml" / "owl" | 单元测试、迁移脚本、从配置中心读取后解析 |
...
# 方式二：知识服务（需配置 knowledge_service_endpoint）
loader.load_from_service(view_id="employee_opportunity_view")
# 或按对象加载
loader.load_from_service(object_ids=["sales_person", "sales_business_opportunity"])

# 方式三：内存内容（如从配置中心或迁移脚本得到）
raw = {"objects": [...], "relations": [...], "functions": [...]}
loader.load_from_content(raw, format="json")
```

##### 6.1.4.2 本体层使用（直接访问 Ontology*）

当需要**直接操作 Ontology* 模型**（如 ObjectViewBuilder 构建 ObjectViewPayload、适配层转换、校验逻辑）时，通过 Loader 获取本体层数据：

| 方法 | 返回 | 用途 |
|------|------|------|
| **get_ontology_class(object_code)** | OntologyClass | 按对象编码获取类定义，含 fields、actions |
| **get_ontology_classes(object_ids=None)** | List[OntologyClass] | 获取全部或指定对象；ObjectViewBuilder 据此构建 ObjectViewPayload |
| **get_ontology_relations()** | List[OntologyRelation] | 获取对象间关系；供 plan 层生成 join、多步执行 |
| **get_function_config(function_code)** | Function 配置（api_schema 等） | Action 执行时查找 API 定义 |

```python
# ObjectViewBuilder 内部示例：从本体层构建 ObjectViewPayload
loader = OntologyLoader()
loader.load_from_path("resources/ontology/opportunity.yaml")

classes = loader.get_ontology_classes(object_ids=["sales_business_opportunity"])
relations = loader.get_ontology_relations()
# 将 OntologyClass[] + OntologyRelation[] 转为 ObjectViewPayload.sources/objects/relations
payload = object_view_builder.build(classes, relations, view_id="...")
```

##### 6.1.4.3 核心层使用（View / Object / Action）

当需要**执行查询或动作**时，通过 Loader 获取核心层实体：

| 方法 | 返回 | 用途 |
|------|------|------|
| **get_object(object_code)** | Object | 获取对象，调用 query()、invoke_action()、get_relations() |
| **get_view(view_id)** | View | 获取视图，调用 query()、invoke_object_action() |
| **get_action(object_code, action_code)** | Action | 直接获取动作，调用 execute()、get_schema() |

```python
# 核心层完整示例：加载 → 获取 Object → 执行动作
...
print(obj.get_description())                    # 自生说明
schema = obj.get_action("query_opportunity_by_emp").get_schema()  # MCP inputSchema
...
flowchart TB
    ...
    subgraph 本体层API
        OC[get_ontology_class]
        OCs[get_ontology_classes]
        OR[get_ontology_relations]
    end
...
> **完整示例**：含 YAML 本体示例、Ontology* 内存结构、自然语言查询与 MCP tools/call 流程，见 `docs/plans/2026-03-06-ontology-to-core-flow-example-design.md`。
...
#### 6.8.1 动作参数术语映射（Term 映射）

除字段名映射外，部分动作参数还与**术语/枚举（Term）** 绑定，例如合同状态、订单阶段等。在这种场景下：

- **OntologyActionParam** 中需描述参数绑定的术语集信息，例如：`term_set`（术语集 ID/枚举类型）、`term_code_field`（物理代码字段）、`term_label_field`（展示标签字段，可选）。
- 调用方通常按「业务术语/标签」传值（如“已签约”“已生效”），而下游 API/SQL 需要的是标准代码值（如 `SIGNED`、`EFFECTIVE`）。
- 在动作执行前，由 **术语转换组件（TermResolver）** 根据 `term_set` 将**逻辑参数值（标签/别名）转换为标准术语代码值**，再写入实际的物理入参（API body / query 参数 / SQL 绑定变量）。
- 若找不到对应术语或值非法，应返回结构化错误，提示可选值范围；对于 MCP 工具可映射为工具错误，对 REST 接口则返回 4xx + 错误详情。

术语映射与字段名映射一起，保证：**调用方始终使用业务一致的逻辑字段名与术语值，而执行层始终使用标准化的物理字段名与代码值。**

### 6.9 架构层级与代码目录

#### 6.9.1 架构层级一览

数据服务采用 **SDK + 服务** 分层架构（见第 10 章）：`datacloud_data` 与 `datacloud_data_service` 为同项目下 `src/` 内两个子包，无重复目录。逻辑层级如下：

| 层级 | 子包 | 目录 | 职责简述 |
|------|------|------|----------|
| **接入层** | datacloud_data_service | `api/` | 接收 HTTP/MCP 请求，鉴权、租户解析、设置 InvocationContext；路由分发；统一数据查询调用 SDK；操作类工具转发 |
| **工具层** | datacloud_data_service | `tools/` | 工具注册与列表；统一查询工具封装；操作类工具的 inputSchema 生成、参数映射、术语转换、动作执行编排 |
| **SDK 核心层** | datacloud_data | `view.py`、`object.py`、`action.py`、`relation.py`、`context.py` | View/Object 实体；自然语言查询、自生说明、动作调用；InvocationContext 请求级上下文 |
| **SDK 本体层** | datacloud_data | `ontology/` | 本体解析（OWL/JSON/YAML → 内部模型）；OntologyLoader 产出 View/Object/Relation；术语集加载 |
| **SDK 计划层** | datacloud_data | `plan/` | ObjectViewPayload 构建；LLM 生成计划；计划校验；数据权限改写；步骤转执行任务 |
| **SDK 执行层** | datacloud_data | `executor/`、`sql_executor/` | 调度 API/SQL 步骤；连接池与 SQL 执行；结果转 CSV；从 InvocationContext 获取 token 等 |
| **SDK 聚合层** | datacloud_data | `aggregator/` | 按 aggregation 策略（DIRECT / SQLITE_MEM）产出最终 records |
| **SDK 支撑层** | datacloud_data | `events/`、`csv_storage/` | 事件总线、追踪；CSV 暂存管理 |

#### 6.9.2 代码目录树（完整）

SDK 与服务为**同项目内两个子包**，位于 `datacloud-data/src/` 下，无重复目录。

```
datacloud-data/
├── config/                                # 运行配置（可选，或从配置中心加载）
│   └── settings.yaml
├── resources/                             # 本体与术语资源（唯一真源，见 6.1.1）
│   └── ontology/
│       ├── *.yaml                         # 按类/视图划分的本体文件
│       ├── *.json
│       └── terms/                         # 术语集（code-label 映射）
│           └── *.yaml
├── src/
│   ├── datacloud_data/                # SDK 子包（核心实体 + 执行 + 支撑，见第 10 章）
│   │   ├── __init__.py
│   │   ├── context.py                    # InvocationContext（contextvars，请求级上下文）
│   │   ├── view.py                       # View 实体：query、get_description、invoke_object_action
│   │   ├── object.py                     # Object 实体：query、get_description、invoke_action、get_relations
│   │   ├── action.py                     # Action：execute、get_schema（input/output JSON Schema）
│   │   ├── relation.py                   # Relation 模型：from_object、to_object、cardinality、join_keys
│   │   │
│   │   ├── ontology/                     # 本体层（仅 SDK）
│   │   │   ├── __init__.py
│   │   │   ├── models.py                 # OntologyClass, OntologyField, OntologyRelation, OntologyAction, OntologyActionParam
│   │   │   ├── loader.py                 # OntologyLoader：OWL/JSON/YAML → View/Object/Relation（内部调用 parser 解析格式）
│   │   │   ├── term_loader.py            # 术语集加载（code-label 及别名）
│   │   │   └── adapters/
│   │   │       ├── __init__.py
│   │   │       └── datacloud_adapter.py   # Ontology* → datacloud Entity/Field/Action
│   │   │
│   │   ├── plan/                         # 计划层（仅 SDK）
│   │   │   ├── __init__.py
│   │   │   ├── models.py                 # QueryExecutionPlan、ObjectViewPayload、Step、Aggregation
│   │   │   ├── object_view_builder.py    # OntologyLoader → ObjectViewPayload
│   │   │   ├── query_plan_generator.py   # LLM 生成计划（使用 View/Object.get_description）
│   │   │   ├── plan_validator.py         # 计划校验
│   │   │   ├── data_permission_rewriter.py
│   │   │   └── execution_object_converter.py  # Plan steps → ApiExecTask/SqlExecTask
│   │   │
│   │   ├── executor/                     # 执行层（仅 SDK）
│   │   │   ├── __init__.py
│   │   │   ├── executor.py               # 主执行入口，调度 API/SQL 步骤
│   │   │   ├── api_executor.py           # HTTP 调用（从 InvocationContext 获取 token）
│   │   │   └── sql_executor_adapter.py   # 调用 sql_executor 或外部 SQL 服务
│   │   │
│   │   ├── sql_executor/                 # 数据库连接与 SQL（仅 SDK）
│   │   │   ├── __init__.py
...
│   │   │   └── jdbc_parser.py
│   │   │
│   │   ├── aggregator/                   # 聚合层（仅 SDK）
│   │   │   ├── __init__.py
│   │   │   ├── direct_aggregator.py
│   │   │   └── sqlite_aggregator.py
│   │   │
│   │   ├── events/                       # 支撑层：事件驱动（仅 SDK）
│   │   │   ├── __init__.py
│   │   │   ├── bus.py
│   │   │   ├── events.py
│   │   │   ├── handlers.py               # 事件处理，调用 plan/executor/aggregator
│   │   │   └── tracing.py
│   │   │
│   │   └── csv_storage/                 # 支撑层：CSV 暂存（仅 SDK）
│   │       ├── __init__.py
│   │       └── manager.py
│   │
│   └── datacloud_data_service/           # 服务子包（接入 + 工具，调用 SDK）
│       ├── __init__.py
│       ├── config.py                     # 配置加载
│       │
│       ├── api/                          # 接入层：REST/MCP 入口
│       │   ├── __init__.py
│       │   ├── routes.py                 # REST 路由注册
│       │   ├── query.py                  # POST /api/v1/query，设置 InvocationContext，调用 SDK View.query
│       │   ├── federated.py              # POST /api/v1/query/federated
│       │   ├── skills.py                 # GET /api/v1/skills/package
│       │   └── mcp_handler.py            # MCP tools/list、tools/call 入口与分流
│       │
│       └── tools/                        # 工具层：统一查询 + 操作类工具
│           ├── __init__.py
│           ├── registry.py               # 工具注册，tools/list 返回（含 View/Object 动作）
│           ├── unified_query.py          # unified_data_query 工具，调用 SDK
│           ├── action_tool_generator.py # 从 Action.get_schema 生成 MCP inputSchema
│           ├── action_executor.py        # 操作类执行编排，调用 SDK Object.invoke_action / View.invoke_object_action
│           ├── input_schema_builder.py  # params → MCP inputSchema 转换
│           ├── param_mapper.py           # arguments → 逻辑参数名 + mapping_path → 物理请求
│           └── term_resolver.py          # 术语值转换：标签/别名 → 标准 code
│
├── tests/
│   ├── datacloud_data/               # SDK 单元/集成测试
│   └── datacloud_data_service/           # 服务层测试（api、tools）
├── pyproject.toml
└── README.md
```
...
**datacloud_data**（`src/datacloud_data/`）

| 层级 | 目录 | 文件 | 职责 |
|------|------|------|------|
| SDK 核心层 | （根） | context.py | InvocationContext：contextvars 请求级上下文；set/get tenant_id、token、cookie 等 |
| | | view.py | View 实体：query(question)、get_description()、invoke_object_action(object_id, action_code, params) |
| | | object.py | Object 实体：query、get_description、invoke_action、get_action_schema、get_relations |
| | | action.py | Action：execute(params)、get_schema() → {input, output} JSON Schema |
| | | relation.py | Relation 模型：from_object、to_object、cardinality、join_keys |
| SDK 本体层 | ontology/ | models.py | OntologyClass、OntologyField、OntologyRelation、OntologyAction、OntologyActionParam |
| | | loader.py | OntologyLoader：OWL/JSON/YAML → View、Object、Relation（内部解析格式为 Ontology* 模型） |
| | | term_loader.py | 术语集加载（code、label、aliases） |
| | | adapters/datacloud_adapter.py | Ontology* → datacloud Entity/Field/Action |
| SDK 计划层 | plan/ | models.py | QueryExecutionPlan、ObjectViewPayload、Step、Aggregation |
| | | object_view_builder.py | OntologyLoader → ObjectViewPayload |
| | | query_plan_generator.py | LLM 生成计划，使用 View/Object.get_description |
| | | plan_validator.py | 计划校验 |
| | | data_permission_rewriter.py | 租户/权限注入 |
| | | execution_object_converter.py | Plan steps → ApiExecTask/SqlExecTask |
| SDK 执行层 | executor/ | executor.py | 调度 API/SQL 步骤，结果落 CSV |
| | | api_executor.py | HTTP 调用，从 InvocationContext 获取 token |
| | | sql_executor_adapter.py | 调用 sql_executor 或外部 SQL 服务 |
| SDK 执行层 | sql_executor/ | models.py | SqlExecTask、DataSourceConfig |
| | | data_source_manager.py | 数据源配置与连接信息 |
| | | connection_pool.py | 连接池管理 |
| | | sql_executor.py | SQL 执行与结果集读取 |
| | | result_converter.py | 结果集 → CSV/records |
| | | jdbc_parser.py | JDBC URL 解析与驱动映射 |
| SDK 聚合层 | aggregator/ | direct_aggregator.py | strategy=DIRECT，从指定 step 的 CSV 产出 |
| | | sqlite_aggregator.py | strategy=SQLITE_MEM，联邦 SQL 聚合 |
| SDK 支撑层 | events/ | bus.py、events.py、handlers.py、tracing.py | 事件总线、事件类型、订阅注册、链路追踪 |
| | csv_storage/ | manager.py | CSV 路径管理、读写 |

**datacloud_data_service**（`src/datacloud_data_service/`）

| 层级 | 目录 | 文件 | 职责 |
|------|------|------|------|
| 接入层 | api/ | routes.py | REST 路由注册 |
| | | query.py | POST /api/v1/query；设置 InvocationContext；发布事件或调用 SDK View.query |
| | | federated.py | POST /api/v1/query/federated |
| | | skills.py | GET /api/v1/skills/package |
| | | mcp_handler.py | MCP 入口；tools/call 按工具名分流（unified_data_query → 事件总线，action_code → ActionExecutor） |
| 工具层 | tools/ | registry.py | 工具列表（统一查询 + 操作类，含 View/Object 动作） |
| | | unified_query.py | 统一数据查询工具封装，调用 SDK |
| | | action_tool_generator.py | 从 Action.get_schema 生成 MCP 工具 name/description/inputSchema |
| | | action_executor.py | 操作类执行编排；调用 SDK Object.invoke_action / View.invoke_object_action |
| | | input_schema_builder.py | OntologyActionParam → JSON Schema properties/required |
| | | param_mapper.py | arguments 逻辑名映射；逻辑值 + mapping_path → API/SQL 物理参数 |
| | | term_resolver.py | 按 term_set 将标签/别名转换为标准 code；非法值返回错误与可选值 |

---

### 6.10 动作执行详细设计

本节描述**操作类动作**从 OntologyAction 到 API/SQL 执行的完整设计，包括参数模型、参数处理流水线（字段名映射、术语转换、实体校验）、以及各环节对应的层级与文件。

#### 6.10.1 OntologyAction 与 OntologyActionParam 模型

- **OntologyAction**（在 `ontology/models.py`）  
  - 字段示例：`action_id`、`action_code`（工具名）、`name`、`description`、`belong_class`、`params: List[OntologyActionParam]`、`function_refs`（绑定的 API/SQL 配置）、`mapping_path` 等。
- **OntologyActionParam**（在 `ontology/models.py`）  
  - 字段：`param_code`、`param_name`、`direction`（IN/OUT/INOUT）、`param_type`、`required`、`default_value`、`mapping_path`（如 `$.requestBody.status`）；  
  - **术语绑定**：`term_set`（术语集 ID）、`term_code_field`、`term_label_field`（可选）；  
  - **实体/校验引用**（可选）：`entity_ref`、`field_ref`，用于对接 datacloud Field 的校验（类型、正则、枚举、必填）。

#### 6.10.2 动作参数处理流水线

操作类工具执行时（`ActionExecutor`，见 `tools/action_executor.py`），参数按以下顺序处理：

1. **入参接收**  
   - MCP `tools/call` 的 `arguments` 或 REST 请求体 → 原始 key-value（可能为逻辑名或别名）。

2. **逻辑名映射**（`tools/param_mapper.py`）  
   - 根据 OntologyActionParam 的 `param_code`、`aliases`，将 arguments 的 key 标准化为逻辑参数名；  
   - 与 6.8 的「同义不同名」一致，保证后续步骤统一使用逻辑名。

3. **术语值转换**（`tools/term_resolver.py`）  
   - 对声明了 `term_set` 或 `ext_attrs.term_type_code` 的参数，按术语集将**标签/别名**转为**标准 code**；  
   - 若值非法或未找到映射，返回结构化错误（参数名、原始值、可选值列表）；MCP 映射为工具错误，REST 为 4xx。

4. **实体/字段校验**（可选，通过 `ontology/adapters/datacloud_adapter` 或独立校验器）  
   - 对声明了 `entity_ref`/`field_ref` 的参数，可调用 datacloud 的 Field/validators 做类型、长度、枚举、必填等校验；  
   - 校验失败时同样返回结构化错误。

5. **物理参数写入**（`tools/param_mapper.py`）  
   - 将校验后的逻辑名+值，按各 OntologyActionParam 的 `mapping_path` 写入 API 请求（body/query/path/header）或 SQL 绑定变量。

6. **执行**  
   - 构造 `ApiExecTask` 或 `SqlExecTask`，交由 `executor/api_executor.py` 或 `sql_executor_adapter` 执行；结果按 OUT 参数的 mapping_path 解析并返回。

#### 6.10.3 动作执行涉及的文件与层级

| 环节 | 层级 | 文件 | 说明 |
|------|------|------|------|
| 工具定义与 inputSchema | 工具层 | action_tool_generator.py、input_schema_builder.py | 从 OntologyAction/Param 生成 MCP 工具与 enum（若有 term_set） |
| 执行编排 | 工具层 | action_executor.py | 串联参数流水线 → 构造 Task → 调 Executor |
| 逻辑名与物理映射 | 工具层 | param_mapper.py | arguments → 逻辑名；逻辑值 + mapping_path → 请求体/参数 |
| 术语转换 | 工具层 | term_resolver.py | 标签/别名 → 标准 code；依赖 ontology/term_loader 或术语配置 |
| 术语数据 | 本体层 | ontology/term_loader.py；resources/ontology/terms/ | 术语集加载 |
| 实体/字段校验 | 本体层 | ontology/adapters/datacloud_adapter.py（或 validators 桥接） | 可选，调用 datacloud Field/validators |
| 参数与动作模型 | 本体层 | ontology/models.py | OntologyAction、OntologyActionParam |
| HTTP/SQL 执行 | 执行层 | executor/api_executor.py、sql_executor/ | 接收已填充好的 Task，执行并返回 |

以上设计保证：**调用方使用业务语义一致的字段名与术语值，执行层使用标准化的物理字段名与代码值，且参数转换与校验在工具层与本体层有清晰的文件归属。**
---
...
动作参数除了字段名映射（见 6.8）外，还可能绑定到**术语/枚举（Term）**。为支持“调用方按业务术语传值、执行层按标准代码执行”，ActionExecutor 在执行前需完成以下步骤：

- **arguments → 逻辑参数名**：根据 action.params 的 `param_code` / `aliases`，将 `params.arguments`（工具入参）标准化为逻辑参数名。
- **逻辑值 → 标准术语值**：当某参数声明了 `ext_attrs.term_type_code` 或 OntologyActionParam 中存在 `term_set` 时，调用术语转换组件（TermResolver），按术语集将“标签/别名值”（如“已签约”）转换为标准代码值（如 `SIGNED`）。
- **标准值 → 物理入参**：将转换后的值按 `mapping_path` 写入下游 API/SQL 的请求参数（body/query/path/header 或 SQL 绑定变量）。
- **校验与错误返回**：若术语不存在或值非法，返回结构化错误（包含参数名、原始值、可选值列表），对于 MCP 工具映射为工具错误，对于 REST 接口则返回 4xx。

通过上述步骤，调用方始终以业务语义稳定的术语与字段名使用工具，而下游系统始终接收标准化的字段名与代码值。

#### 8.5.5 tools/list 中的操作类工具
...
本节补充基于 OWL/JSON/YAML 构造**视图（View）**和**对象（Object）**实体的 SDK 设计，以及 SDK 与 data-service 的分层关系。SDK 支持进程内与远程调用，data-service 使用 SDK 提供 MCP 能力。

### 10.1 整体架构与目录结构

采用**同项目内双子包**：`datacloud_data` 与 `datacloud_data_service` 为 `datacloud-data/src/` 下两个子包，无重复目录。执行层与支撑层（plan、executor、aggregator、events、csv_storage）均置于 SDK 内。

```
datacloud-data/
├── src/
│   ├── datacloud_data/                # SDK 子包（核心 + 执行 + 支撑）
│   │   ├── context.py、view.py、object.py、action.py、relation.py
│   │   ├── ontology/                     # 本体解析与模型
│   │   ├── plan/                         # 计划层
│   │   ├── executor/、sql_executor/      # 执行层
│   │   ├── aggregator/                   # 聚合层
│   │   ├── events/                       # 事件总线
│   │   └── csv_storage/                  # CSV 暂存
│   │
...
### 10.2 SDK 核心能力（datacloud-data-sdk）

| 组件 | 职责 |
|------|------|
| **InvocationContext** | 使用 `contextvars` 实现请求级上下文；`set(tenant_id, token, cookie, ...)`；`get()` 供 View/Object/Action 使用；动态 token/cookie 由入口统一设置，视图和对象可全局获取 |
| **View** | 从 Ontology 构建；`query(question)` 自然语言查询；`get_description()` 返回 MD（含能力、对象动作、对象关联）；`invoke_object_action(object_id, action_code, params)` 通过视图调用对象动作 |
| **Object** | 从 Ontology 构建；`query(question)`；`get_description()`（含关联对象）；`invoke_action(action_code, params)`；`get_action_schema(action_code)`；`get_relations()` 返回关联关系 |
| **Relation** | 对象间关联：`from_object`、`to_object`、`cardinality`、`join_keys` 等；供 View 自生说明和 QueryPlanGenerator 使用 |
| **Action** | 封装 OntologyAction；`execute(params)`；`get_schema()` → `{input: {...}, output: {...}}` 结构化 schema |
| **OntologyLoader** | 解析 OWL/JSON/YAML，产出 View、Object、Relation 实例 |

**视图与对象的能力边界**：

- **视图**：仅提供自然语言查询、自生说明；不定义自定义动作；可通过视图使用对象的动作。
- **对象**：有自然语言查询、自生说明，以及 OWL/JSON/YAML 中定义的自定义动作。

**自生说明与动作 schema**：

- **自生说明（Markdown）**：面向外部/人类，描述视图能力、对象动作、对象间关系、动作的出入参等，类似 skill 的 md 文档。
- **动作 schema（结构化）**：通过 `Action.get_schema()` 获取，提供 input/output 的 JSON Schema，供 MCP、SDK 等程序化使用。

### 10.3 对象关联（Relation）

| 能力 | 说明 |
|------|------|
| **View.get_description()** | MD 中描述：包含哪些对象、对象间关联（如「员工-商机-合同」）、可用的对象动作 |
| **Object.get_relations()** | 返回该对象参与的关联列表（`Relation` 或等价结构） |
| **Object.get_description()** | MD 中描述：字段、动作、**关联对象**（如「商机关联合同、员工」） |
| **OntologyLoader** | 解析本体中的 `relations`，构建 `Relation` 并挂到 View/Object |
| **QueryPlanGenerator** | 使用 relations 生成多表 join、多步执行计划 |
...
@dataclass
class Relation:
    from_object: str      # 源对象 ID
    to_object: str        # 目标对象 ID
    cardinality: str      # ONE_TO_MANY, MANY_TO_ONE, ONE_TO_ONE, MANY_TO_MANY
    join_keys: list       # [{from_field, to_field}, ...]
    description: str      # 业务含义（可选）
```

### 10.4 数据流与 InvocationContext

**自然语言查询流程**：

```
请求进入 → 设置 InvocationContext(token, cookie, tenant_id, ...)
    → View.query(question) 或 Object.query(question)
    → QueryPlanGenerator（从 Context 获取配置，用 get_description() 生成 prompt）
    → PlanValidator
    → Executor（从 Context 获取 token 调用 API）
    → Aggregator
    → 返回 records
```
...
```
请求进入 → 设置 InvocationContext
    → View.invoke_object_action(obj_id, action_code, params) 或 Object.invoke_action(action_code, params)
    → Action.execute(params)（从 Context 获取 token 等）
    → ApiExecutor / SqlExecutor
    → 返回结果
```

**InvocationContext 使用约定**：

| 场景 | 设置时机 | 获取方 |
|------|----------|--------|
| REST 入口 | 中间件/拦截器，解析 Header 后 | View、Object、Executor |
| MCP 入口 | tools/call 处理前 | 同上 |
| SDK 进程内调用 | 调用方在入口处 `InvocationContext.set(...)` | 同上 |

```python
# 使用示例
with InvocationContext(tenant_id="t1", token="xxx", cookie="yyy"):
    result = view.query("查询邹海天签了合同的商机")
```
...
| 配置项 | 说明 | 来源 |
|--------|------|------|
| tenant_id, user_id, session_id | 租户/用户/会话 | InvocationContext |
| token, cookie | 动态鉴权 | InvocationContext |
| data_source_config | 数据源连接 | 配置中心或 Context |
| llm_config | LLM 调用配置 | 配置或 Context |
...
| 场景 | 处理方式 |
|------|----------|
| 动作参数校验失败 | 返回结构化错误（参数名、原因、可选值） |
| 术语转换失败 | 同上 |
| SQL/API 执行失败 | 记录 trace，返回统一错误格式 |
| Context 未设置 | 抛出明确异常，提示需先设置 InvocationContext |
| 动作不存在 | 返回 404 或等价错误 |
...
| 层级 | 内容 |
|------|------|
| SDK 单元测试 | View/Object/Action 的 get_description、get_schema、Relation 解析 |
| SDK 集成测试 | OntologyLoader 解析 OWL/JSON/YAML → View/Object；Context 隔离 |
| 服务层测试 | MCP tools/list、tools/call 路由；REST 与 Context 设置 |
| 端到端测试 | 自然语言查询、动作调用、多租户隔离 |
...
- **本体驱动**：从知识服务获取 OWL/JSON/YAML 本体，**解析为内部本体模型（OntologyClass/Field/Relation/Action）**，再构建 ObjectViewPayload，由 LLM 生成 QueryExecutionPlan；**本体唯一真源在 data-service**，datacloud 通过适配层融入（见 6.1.1～6.1.3）。
...
- **SDK 与分层**：**datacloud-data-sdk** 提供 View、Object、Relation、Action、InvocationContext 及执行层；data-service 使用 SDK 提供 MCP/REST；SDK 支持进程内与远程调用（见第 10 章）。
- **操作类 MCP 工具**：由 OWL/JSON/YAML 的 actions 生成 inputSchema，执行时直接构造 ApiExecTask 调用 API 返回，不经过查询流水线。

### 11.2 实施优先级

1. **Phase 1**：**datacloud-data-sdk** 基础（OntologyLoader、View、Object、Relation、Action、InvocationContext）、统一数据源配置、ObjectViewPayload 构建、QueryExecutionPlan 生成、**SqlExecutor 模块**（连接池 + SQL 执行）、CSV + SQLite 聚合、**操作类 MCP 工具**（ActionToolGenerator + ActionExecutor）
2. **Phase 2**：data-service 接入 SDK、事件总线（内存同步）、计划校验重试、数据权限改写、**事件链路追踪与模块耗时监控**（日志模式）
