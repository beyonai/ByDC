...
{
  "dimensions": [
    {"field": "field_code", "group_op": "month|self|range|...", "buckets": [...]},
  ],
  "metrics": [
    {"field": "field_code", "agg": "sum|avg|min|max|count|count_distinct", "as": "alias"},
    {"agg": "count_all", "as": "alias"}
  ],
  "filters":  [{"field": "field_code", "op": "...", "value": ...}],
  "having":   [{"field": "metrics.as 别名", "op": "...", "value": ...}],
  "order_by": [{"field": "metrics.as 别名 或维度field_code", "direction": "asc|desc"}],
  "limit":    100
}

特点：
- dimensions[i].field / metrics[i].field / filters[i].field 直接使用 field_code
...
def _check_snapshot_cross_period_sum(
    ...
    入参 dimensions[i].field / metrics[i].field 均为 field_code。
    """
...
class ComputeExecutor:
    ...
    协议（§3.2.3 字段编码版）：
    - dimensions[i].field / metrics[i].field / filters[i].field 均接受 field_code
    - 不再接受字段中文名；传中文名时校验层报 UNSUPPORTED_FIELD
    - having[i].field 仍为 metrics.as 别名
    - 支持 formula 字段展开、period substr 优化、snapshot_metric 跨账期 SUM 拦截
    """
    ...
    async def execute(
        ...
        Args:
            object_code: 对象编码
            arguments:   符合 §3.2.3 协议的入参（field 均为 field_code）
        ...
        # ── 2. snapshot_metric 跨账期 SUM 前置检测（直接用 field_code） ────────
