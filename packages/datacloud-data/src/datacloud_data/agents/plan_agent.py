...
from datacloud_data.exceptions import PlanGenerationError
from datacloud_data.plan.models import ObjectViewPayload, QueryExecutionPlan, parse_plan
...
class PlanAgent:
    ...
    async def _generate_node(self, state: PlanAgentState) -> dict[str, Any]:
        ...
        object_view_json = json.dumps(
            _serialize_payload(payload, term_loader=state.get("term_loader")),
            ensure_ascii=False,
            indent=2,
        )

        user_message = _build_user_message(
            object_view_json, question, validation_errors
        )
        messages = [
