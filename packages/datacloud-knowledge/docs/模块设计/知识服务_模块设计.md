...
CREATE TABLE term_name (
    ...
    name_embedding VECTOR(1024)            COMMENT '名称的向量特征，用于模糊别名语义捕获',
    search_scope   JSONB         DEFAULT '{}' COMMENT '别名标签属性，用于控制作用域（如特定员工对该术语的专属别名）',
    created_time TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_time TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (term_id) REFERENCES term(term_id)
);

-- 名称文本索引：支持精确匹配和前缀查询 (B-Tree)
CREATE INDEX idx_tn_name_text ON term_name(name_text);

-- 名称全文检索索引 (GIN / BM25：支持对中长别名的切词匹配)
CREATE INDEX idx_tn_name_fts ON term_name USING GIN (to_tsvector('simple', COALESCE(name_text, '')));

-- 术语ID索引：支持按术语查询所有名称
CREATE INDEX idx_tn_term ON term_name(term_id);

-- 标签属性 GIN 索引：支持别名级别的控制查询
CREATE INDEX idx_tn_tags ON term_name USING GIN (search_scope);

-- 名称向量索引 (基于 pgvector 支持别名的泛化语义查找)
-- CREATE INDEX idx_tn_embedding ON term_name USING ivfflat (embedding vector_cosine_ops);
```
...
| name_embedding   | VECTOR(1024)       | 否                 | 名称的向量特征，用于应对口语化泛指和语义关联 |
| search_scope     | JSONB              | 否                 | 别名标签，用于控制个人、组织的专有别名作用域 |
| created_time     | TIMESTAMP          | 是                 | 创建时间 |
| updated_time     | TIMESTAMP          | 是                 | 更新时间 |
