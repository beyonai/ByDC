...
CREATE TABLE term_name (
    ...
    embedding    VECTOR(1024)            COMMENT '名称的向量特征，用于模糊别名语义捕获',
    name_tags    JSONB         DEFAULT '{}' COMMENT '别名标签属性，用于控制作用域（如特定员工对该术语的专属别名）',
    created_time TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_time TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (term_id) REFERENCES term(term_id)
);
...
CREATE INDEX idx_tn_tags ON term_name USING GIN (name_tags);
...
| embedding        | VECTOR(1024)       | 否                 | 名称的向量特征，用于应对口语化泛指和语义关联 |
| name_tags        | JSONB              | 否                 | 别名标签，用于控制个人、组织的专有别名作用域 |
| created_time     | TIMESTAMP          | 是                 | 创建时间 |
| updated_time     | TIMESTAMP          | 是                 | 更新时间 |
