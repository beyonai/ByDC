...
def _render_mapper(class_name: str, entity_code: str, fields: list[dict[str, Any]]) -> str:
    ...
    body: list[str] = [
        ...
        '        """按 id 更新记录。"""',
        "        return self._loader.update_by_id(",  # type: ignore[return-value]
        f'            "{entity_code}", obj.to_dict()',
        "        )",
        "",
        "    def delete_by_id(self, id: int) -> bool:",
        '        """按主键删除记录。"""',
        "        return self._loader.delete_by_id(",  # type: ignore[return-value]
        f'            "{entity_code}", id',
        "        )",
        "",
        "    def select(self, q: Any) -> dict[str, Any]:",
        '        """条件查询，返回 {records, total, meta}。"""',
        "        return self._loader.query(",  # type: ignore[return-value]
        ...
        '        """条件计数。"""',
        "        return self._loader.count(",  # type: ignore[return-value]
        f'            "{entity_code}", q.to_payload() if hasattr(q, "to_payload") else q',
        "        )",
        "",
        "    def agg(self, a: Any) -> dict[str, Any]:",
        '        """聚合查询，返回 {records, total, meta}。"""',
        "        return self._loader.aggregate(",  # type: ignore[return-value]
