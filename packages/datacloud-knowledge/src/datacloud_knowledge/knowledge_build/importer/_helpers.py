...
def _execute_values(
    ...
    for i in range(0, len(argslist), page_size):
        ...
        cur.execute(cast(Any, prefix + values_clause), flat_params)
...
def _lookup_term_ids_by_term_keys(
    cur: Cursor,
    keys: list[tuple[str | None, str, str | None, str]],
) -> dict[tuple[str | None, str, str | None, str], str]:
    """按 (library_id, term_type_code, parent_term_id, term_code) 查找 term.term_id。"""
    uniq = list(dict.fromkeys(keys))
    if not uniq:
        return {}
    params: list[Any] = []
    clauses: list[sql.SQL] = []
    for library_id, term_type_code, parent_term_id, term_code in uniq:
        clauses.append(
            sql.SQL(
                "(library_id IS NOT DISTINCT FROM %s AND term_type_code = %s AND parent_term_id IS NOT DISTINCT FROM %s AND term_code = %s)"
            )
        )
...
def _lookup_term_ids_by_parent_keys(
    cur: Cursor,
    keys: list[tuple[str | None, str, str | None, str]],
) -> dict[tuple[str | None, str, str | None, str], str]:
    """按 (library_id, term_type_code, parent_term_id, term_code) 查找 term.term_id。"""
    ...
    for library_id, term_type_code, parent_term_id, term_code in uniq:
        clauses.append(
            sql.SQL(
                "("
                "library_id IS NOT DISTINCT FROM %s AND "
                "term_type_code = %s AND "
                "parent_term_id IS NOT DISTINCT FROM %s AND "
                "term_code = %s"
                ")"
            )
        )
