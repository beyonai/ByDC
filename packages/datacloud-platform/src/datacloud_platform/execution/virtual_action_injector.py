...
def inject_virtual_actions(loader: object) -> None:
    ...
    # ── 1. 对象级虚拟动作 ─────────────────────────────────────────────────────
    for cls in loader._classes.values():  # type: ignore[attr-defined]
...
def _attach_term_values(fields: list[Any], term_loader: Any | None) -> None:
...
def _ensure_analytic_meta(fields: list[Any]) -> None:
    ...
    for f in fields:
        ...
        f.filter_ops, f.group_ops, f.aggregate_ops, f.required_filter_group = (
            derive_field_ops(f.analytic_role, f.analytic_kind, term_type=term_type)
        )
        f.secondary_role = infer_secondary_role(f.analytic_role, f.analytic_kind)


def _required_filter_groups(fields: list[Any]) -> list[str]:
    """收集字段中出现的强制过滤组。"""
    groups: list[str] = []
    seen: set[str] = set()
    for f in fields:
...
def _has_measure(fields: list[Any]) -> bool:
    return any(
...
def _inject_db_object_actions(
    cls: Any, existing_codes: set[str], registry: Any
) -> None:
...
def _inject_dynamic_table_object_actions(
    cls: Any, existing_codes: set[str], registry: Any
) -> None:
    ...
    operation_specs = [
        (
            ...
            f"insert_{obj_code}",
            f"新增{obj_name}",
            build_insert_schema(obj_name, fields),
        ),
        (
            "update",
            f"update_{obj_code}",
            f"修改{obj_name}",
            build_update_schema(obj_name, fields),
        ),
        (
            "delete",
            f"delete_{obj_code}",
            f"删除{obj_name}",
            build_delete_schema(obj_name, fields),
        ),
    ]
...
def _inject_view_actions(loader: Any, registry: Any) -> None:
    ...
    for view_id, scene in scenes.items():
        # 只处理 DB 视图（有对象列表的场景）
        raw_objects = (
            scene.get("object_codes")
            or scene.get("objects")
            or scene.get("object_ids")
            or []
        )
        if not raw_objects:
            continue
...
def _extract_view_fields(scene: dict[str, Any], loader: Any) -> list[Any]:
    """
        ...
        for rf in raw_fields:
            if isinstance(rf, ViewFieldMeta):
                if rf.secondary_role is None:
                    rf.secondary_role = infer_secondary_role(
                        rf.analytic_role, rf.analytic_kind
                    )
