...
class _McpGatewayContext:
    ...
    @asynccontextmanager
    async def sub_step(self, title: str) -> AsyncIterator[tuple[str, str]]:
        message_id = self.generate_message_id()
        parent_message_id = self.message_id
        await self.emit_state(
            title, message_id=message_id, parent_message_id=parent_message_id
        )
...
def _get_loader_runtime() -> Any:
    runtime = (
        _loader_runtime_ref() if callable(_loader_runtime_ref) else _loader_runtime_ref
    )
    return runtime
...
async def _get_loader_snapshot(reason: str) -> LoaderSnapshot | None:
    ...
    if runtime is not None:
        snapshot: LoaderSnapshot | None = await runtime.ensure_fresh(reason)
        if (
...
def _validate_tool_arguments(
    ...
    *,
    ...
    def _validate_value(value: Any, node: dict[str, Any], path: str) -> None:
        ...
        if expected_type == "object":
            ...
            properties: dict[str, Any] = (
                node.get("properties")  # type: ignore[assignment]
                if isinstance(node.get("properties"), dict)
                else {}
            )
            required: list[str] = (
                node.get("required")  # type: ignore[assignment]
    ...
    return [
        TextContent(
            type="text",
            text=format_input_validation_error(get_current_language(), errors[0]),
        )
    ]
...
def _create_mcp_app() -> tuple[Server, StreamableHTTPSessionManager]:
    ...
    server = Server("datacloud-platform")
    ...
    async def list_tools() -> list[Tool]:
        ...
        try:
            ...
            from datacloud_platform.execution.registry import ToolRegistry
    ...
    async def call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
        ...
        try:
            ...
            if snapshot is None:
                return [
                    TextContent(
                        ...
                        text=json.dumps(
                            {
                                "error": format_loader_not_initialized(
            ...
            if name == "unified_data_query":
                ...
                text = _render_tool_call_text(
                    result, include_detail=include_tool_call_detail
                )
            ...
            if scope is None:
                return [
                    TextContent(
                        ...
                        text=json.dumps(
                            {
                                "error": format_unknown_tool(
                                    get_current_language(), name
                                )
                            },
                            ensure_ascii=False,
                        ),
            ...
            if scope_type == "view":
                ...
                text = _render_tool_call_text(
                    payload, include_detail=include_tool_call_detail
                )
                return [TextContent(type="text", text=text)]
            else:
...
def create_mcp_asgi_app(
    ...
    async def handle_streamable_http(
        ...
        view_id = headers.get("x-view-id", "").strip() or (
            view_ids[0] if view_ids else ""
        )
        ...
        ctx_kwargs = {
            ...
            "tool_call_detail": _parse_bool_header(
                headers.get(_TOOL_CALL_DETAIL_HEADER)
            ),
