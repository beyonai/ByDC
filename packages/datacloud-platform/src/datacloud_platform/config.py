...
class Settings(BaseSettings):
    ...
    llm_temperature: float = Field(
        default=0.0, validation_alias="DATACLOUD_LLM_TEMPERATURE"
    )
    ontology_path: str = Field(
        default="resources/ontology/crm_demo/objects_registry.json",
        validation_alias="DATACLOUD_ONTOLOGY_PATH",
    )
    csv_base_dir: str = Field(
        default="./tmp", validation_alias="DATACLOUD_CSV_BASE_DIR"
    )
    ...
    max_plan_retries: int = Field(
        default=2, validation_alias="DATACLOUD_MAX_PLAN_RETRIES"
    )
    ...
    trace_enabled: bool = Field(
        default=False, validation_alias="DATACLOUD_TRACE_ENABLED"
    )
