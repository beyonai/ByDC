...
超级分析智能体（Super Analysis Agent）是 dataCloud 的核心智能体服务，基于 LangGraph 实现从自然语言到数据洞察的完整主链路。
...
`datacloud-analysis` 是 dataCloud 平台的分析智能体核心包，负责将用户的自然语言问题转化为结构化数据查询并返回洞察结果。

核心能力：

- **意图识别**：区分命令型请求与 ReAct 推理型请求，命令直接路由，推理进入多轮循环
- **知识增强**：在 LLM 调用前注入本体知识片段和术语提示，提升参数抽取准确率
- **贪心参数抽取**：在工具调用前尽可能完成任务识别、字段映射、复杂条件识别，减少工具调用轮次
- **渐进降级执行**：优先走 DSL 标准路径，遇到复杂条件自动降级到 text2SQL
- **澄清对话**：歧义无法自动消解时，通过 `interrupt()` 暂停图执行，等待用户确认后恢复
- **会话持久化**：基于 PostgreSQL/OpenGauss 的 Checkpointer，支持跨请求状态恢复
...
| 依赖包 | 用途 |
|--------|------|
| `datacloud-knowledge` | 本体知识检索、术语查询 |
| `datacloud-data` | 本体工具动态生成（query/compute/data_query） |
| `langgraph >= 1.0.10` | 图编排、状态管理、interrupt 机制 |
| `langchain-openai / langchain-anthropic` | LLM 调用适配层 |
| `psycopg[binary,pool]` | PostgreSQL/OpenGauss 连接池 |
