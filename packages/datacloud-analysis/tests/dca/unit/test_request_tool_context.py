...
def test_build_param_link_graph_built_when_anchor_mode_false() -> None:
    ...
    with patch(
        "datacloud_analysis.tools.request_tool_context.ParamLinkGraph", fake_plg_cls
    ):
        ctx = RequestToolContext.build(
            allowed_scope=scope,
            loader=loader,
...
def test_is_object_allowed_priority_object_over_scene() -> None:
    ...
    assert ctx.is_object_allowed("by_customer", scene_id="wrong_scene", library_id="wrong_lib") is True
