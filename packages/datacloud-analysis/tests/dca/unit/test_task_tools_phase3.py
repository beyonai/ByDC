...
def test_load_from_json_resource_directory_supports_camelcase() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert "task_camel_test" in loader._classes, "应支持驼峰格式 objectCode"
...
def test_load_from_json_resource_directory_incremental() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        (obj_dir / "task_new.json").write_text(
            json.dumps(
                {"object_code": "task_new", "object_name": "新对象", "fields": [], "actions": []},
                ensure_ascii=False,
            ),
            encoding="utf-8",
        )
...
def test_ontology_resource_service_create_object_accepts_search_scope_extra() -> None:
    ...
    try:
        ...
        assert "search_scope_extra" in params, "create_object 应有 search_scope_extra 参数"
    except ModuleNotFoundError:
        pytest.skip("datacloud_server 不在当前测试环境")
...
def test_batch_sync_term_names_accepts_search_scope() -> None:
    ...
    assert "search_scope" in sig.parameters, "_batch_sync_term_names 已有 search_scope 参数"
...
def test_build_terms_accepts_search_scope_extra() -> None:
    ...
    assert "search_scope_extra" in sig.parameters, "build_terms 应有 search_scope_extra 参数"
...
def test_create_task_object_schema() -> None:
    ...
    sig = inspect.signature(
        create_task_object.func if hasattr(create_task_object, "func") else create_task_object
    )
    params = sig.parameters
...
def test_create_task_relation_schema() -> None:
    ...
    sig = inspect.signature(
        create_task_relation.func if hasattr(create_task_relation, "func") else create_task_relation
    )
    params = sig.parameters
...
def test_query_task_graph_schema() -> None:
    ...
    sig = inspect.signature(
        query_task_graph.func if hasattr(query_task_graph, "func") else query_task_graph
    )
    params = sig.parameters
