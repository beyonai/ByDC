...
def test_search_ontology_writes_results_to_active_tools() -> None:
    ...
    try:
        with patch(
            ...
            result = search.invoke({"query": "运维链路追踪", "scope": "all", "type": "all", "top_k": 3})
...
def test_search_ontology_returns_candidates_as_string() -> None:
    ...
    mock_hits = [
        ...
        {"objectCode": "ops_metric", "objectName": "指标监控", "resultType": "object", "score": 0.8},
    ]
...
def test_search_ontology_no_results() -> None:
    ...
    with patch(
        ...
        result = search.invoke({"query": "不存在的对象xyz", "scope": "all", "type": "all", "top_k": 3})
