...
# ---------------------------------------------------------------------------
try:
    from datacloud_analysis.config.models import GatewayConfig  # old models module
except ImportError:
...
def gateway_client(
    ...
    with (
        patch("datacloud_analysis.api.client.AgentRunner", return_value=mock_agent_runner),
        patch("datacloud_analysis.api.client.SessionManager", return_value=mock_session_manager),
        patch("datacloud_analysis.api.client.AgentRegistry", return_value=mock_agent_registry),
        patch("datacloud_analysis.api.client.CommandRouter", return_value=mock_command_router),
    ):
        from datacloud_analysis.api.client import GatewayClient
