"""
真实API测试 - 使用pytest验证datacloud-analysis核心功能

运行方式:
    export OPENAI_API_KEY="your-api-key"
    export OPENAI_BASE_URL="https://lab.iwhalecloud.com/gpt-proxy/v1"
    uv run --package datacloud-analysis pytest datacloud-analysis/tests/test_real_api_integration.py -v
...
class TestModelConfig:
    ...
    def test_create_model_with_real_api(self, api_config):
        """测试使用真实API创建模型"""
        from datacloud_analysis.core.model_config import create_model
...
class TestTools:
    ...
    def test_get_business_tools(self):
        """测试获取业务工具"""
        from datacloud_analysis.core.tools import get_business_tools
    ...
    def test_know_tool_invoke(self):
        """测试know工具调用"""
        from datacloud_analysis.core.tools import know
    ...
    def test_query_tool_invoke(self):
        """测试query工具调用"""
        from datacloud_analysis.core.tools import query
    ...
    def test_compute_tool_invoke(self):
        """测试compute工具调用"""
        from datacloud_analysis.core.tools import compute
    ...
    def test_store_tool_invoke(self):
        """测试store工具调用"""
        from datacloud_analysis.core.tools import store
...
class TestSubagents:
    ...
    def test_convert_to_deepagents_format(self):
        """测试转换为deepagents格式"""
        from datacloud_analysis.core.subagents import (
...
class TestQueueSystem:
    ...
    async def test_queue_operations(self):
        """测试队列基本操作"""
        from datacloud_analysis.queue.manager import QueueManager
        from datacloud_analysis.queue.types import (
            QueueSettings,
            QueuedMessage,
            QueueMode,
            DropPolicy,
