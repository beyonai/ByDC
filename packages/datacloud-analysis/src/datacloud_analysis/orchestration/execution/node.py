...
async def execution_node(
    ...
    current_todos = list(state.get("todos") or [])
    plan_todos = [] if current_todos else _build_plan_todos(state, dynamic_tools=dynamic_tools)
    # Prefer the persisted runtime todo snapshot when present so resume-time
    # mutations (for example injected ``__delegate_result__`` payloads) survive
    # into the next execution round instead of being rebuilt from planned_tasks.
    todos = current_todos or plan_todos or _ensure_direct_todo_from_route(
