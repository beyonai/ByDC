...
def _get_next_objects_from_term(
    ...
    try:
        ...
        placeholders_obj = (
            ",".join(f":oc{i}" for i in range(len(object_codes))) if object_codes else "NULL"
        )
        placeholders_sc = (
            ",".join(f":sc{i}" for i in range(len(scene_codes))) if scene_codes else "NULL"
        )
        placeholders_lib = (
            ",".join(f":lib{i}" for i in range(len(library_ids))) if library_ids else "NULL"
        )
            ...
            for _msg in result_dict.get("messages") or []:
                ...
                _source_obj = (
                    next(
                        (
                            obj
                            for obj, tools in _tool_ctx.object_to_tools.items()
                ...
                for _obj_code in _to_add_objs:
                    _obj_tools = _tool_ctx.object_to_tools.get(_obj_code, []) if _tool_ctx else []
                ...
                try:
                    ...
                    if _anchor_update:
                        ...
                        logger.info(
                            "[ChainHint] anchor injected: tool=%s anchor=%s hint_msgs=%d (plg=%s)",
                            _tool_name,
